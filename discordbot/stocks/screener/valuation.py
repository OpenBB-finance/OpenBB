...
async def valuation_command(
    ...
    try:
        ...
        # Debug
        if cfg.DEBUG:
            print(f"!stocks.scr.valuation {preset} {sort} {limit} {ascend}")
        ...
        # Output Data
        df_screen = get_screener_data(preset, "valuation", limit, ascend,)
            ...
            if sort:
                if " ".join(sort) in d_cols_to_sort["valuation"]:
                    ...
                    similar_cmd = difflib.get_close_matches(
                        " ".join(sort), d_cols_to_sort["valuation"], n=1, cutoff=0.7,
                    )
                    if similar_cmd:
