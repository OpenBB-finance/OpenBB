...
export default function AvailableOptions({ label, allOptions }: { label: string, allOptions: string[] }) {
	...
	return (
		<DialogPrimitive.Root onOpenChange={(value) => {
			if (!value) {
				setOptions(allOptions);
			}
		}}>
			<DialogPrimitive.Trigger className="_btn sm:w-fit mb-10">See {label}</DialogPrimitive.Trigger>
			<DialogPrimitive.Overlay className="_modal-overlay" />
			<DialogPrimitive.Content className="_modal">
				<DialogPrimitive.Close tabIndex={-1} className="absolute top-[40px] right-[46px] text-grey-200 hover:text-white rounded-[4px] focus:outline focus:outline-2 focus:outline-grey-500">
				...
				<div className="mt-10 w-full">
					<div className="relative">
						<input
							onChange={(e) => {
								...
								if (query)
									setOptions(searchWithFuse(e.target.value))
					...
					<ul className="h-40 mt-8 space-y-4 text-grey-400 text-sm">
						{options.slice(0, 5).map((option) => <li key={option}>
							{option}
						</li>)}
					</ul>
