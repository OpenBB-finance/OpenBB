...
<HeadTitle title="ETF.HOLDINGS | OpenBB Add-in for Excel Docs" />
...
| provider | Text | Options: fmp, sec, defaults to fmp. | False |
| date | Text | A specific date to get data for. This needs to be _exactly_ the date of the filing. Use the holdings_date command/endpoint to find available filing dates for the ETF. (provider: fmp); A specific date to get data for.  The date represents the period ending.  The date entered will return the closest filing. (provider: sec) | False |
| cik | Text | The CIK of the filing entity. Overrides symbol. (provider: fmp) | False |
| use_cache | Boolean | Whether or not to use cache for the request. (provider: sec) | False |
