```
usage: govp [-l LIMIT] [-s {submitTime,id,depositEndTime,status,type,title,Yes,No}] [--status {voting,deposit,passed,rejected}] [--descend] [-h] [--export {csv,json,xlsx}]
```
...
optional arguments:
  ...
  --status {voting,deposit,passed,rejected}
                        Status of proposal. Default: voting (default: voting)
