...
The OpenBB Terminal is built on a lot of different data sources. The example above collects data from Yahoo Finance. This could be undesirable and therefore we allow for a variety of different data sources. Think of FinancialModelingPrep, Polygon, IEX Cloud, Alpha Vantage and Binance to name a few. Some of these sources require you to set an API Key to connect with their data.
...
The [keys menu](/terminal/usage/guides/api-keys) serves the purpose of providing you to set API keys so that this type of functionality becomes available. For example, in the case of `rot` you will see that the source is "FinnHub".
...
Therefore, you need to acquire an API key from FinnHub through the website and enter the key within the [keys menu](/terminal/usage/guides/api-keys).
...
All of the below examples are stored in the OpenBBUserData folder. This also applies to when you wish to import files, as is for example required within the [Portfolio menu](/terminal/usage/intros/portfolio). To find more information about this folder please have a look [**here**](/terminal/usage/guides/data).
:::
...
Lastly, when you select the `xlsx` option, you can also specify the sheet name with `--sheet-name` which allows multiple datasets to be send to the same Excel file.