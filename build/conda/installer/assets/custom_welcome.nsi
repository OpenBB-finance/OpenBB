...
Function muiExtraPagesAfterWelcome_Create
    ...
    ${NSD_CreateLabel} 10u 10u 280u 120u "Welcome to the OpenBB Platform Installer.$\r$\n$\r$\n$\r$\nThis application will install the latest version of the OpenBB Platform on your computer in a self-contained, Python 3.12, Conda environment.$\r$\n$\r$\n$\r$\nSeveral launch scripts are included: OpenBB CLI, OpenBB API, Jupyter Notebook, IPython, Update, and shortcuts to key folders.$\r$\n$\r$\n$\r$\nIt requires between 1-2 GB of storage space, with a minimum of 4GB RAM. The size estimate provided by the installer is inaccurate.$\r$\n$\r$\n$\r$\nNotes:$\r$\n$\r$\n$\r$\nIn order to install you need access to the Internet.$\r$\n$\r$\n$\r$\nWhen selecting the location of the installation, the path requires a depth of two - i.e, User/MyUserAccount/OpenBB/conda", where "OpenBB" is the target folder, and "conda" is the name of the folder where Conda and the installed environments live.$\r$\n$\r$\n$\r$\nThe path, including username, should not contain any spaces.
