...
def high_short_interest(l_args):
    parser = argparse.ArgumentParser(prog='high_short',
                                     description='''Print top stocks being more heavily shorted.
                                     HighShortInterest.com provides
                                                   a convenient sorted database of stocks which have a short interest
                                                   of over
                                                   20 percent. Additional key data such as the float,
                                                   number of outstanding shares,
                                                   and company industry is displayed. Data is presented for the
                                                   Nasdaq Stock Market,
...
def low_float(l_args):
    parser = argparse.ArgumentParser(prog='low_float',
                                     description='''Print top stocks with lowest float. LowFloat.com provides a
                                     convenient
                                     sorted database of stocks which have a float of under 10 million shares.
                                     Additional key
                                     data such as the number of outstanding shares, short interest, and company
                                     industry is
                                     displayed. Data is presented for the Nasdaq Stock Market, the New York Stock
                                     Exchange,
    ...
    a_low_float_stocks = re.sub('<!--.*?//-->', '', text_soup_low_float_stocks.find_all('td')[3].text,
                                flags=re.DOTALL).split('\n')[2:]
