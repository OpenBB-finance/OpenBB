...
from gamestonk_terminal.helper_funcs import long_number_format, check_positive
...
def quote(l_args, s_ticker):
    parser = argparse.ArgumentParser(
        prog="quote",
        description="""
            Prints actual information about the company which is, among
...
def discounted_cash_flow(l_args, s_ticker):
    parser = argparse.ArgumentParser(
        prog="dcf",
        description="""
            Prints the discounted cash flow of a company over time including
...
def income_statement(l_args, s_ticker):
    parser = argparse.ArgumentParser(
        ...
        description="""
            ...
            income expenses net, Weighted average shs out, Weighted average shs out dil [Source: Financial Modeling Prep]
        """,
    )
...
def key_metrics(l_args, s_ticker):
    ...
    try:
        ...
        df_fa.columns.name = "Fiscal Date Ending"
        df_fa = df_fa.rename(
            index={"Enterprise value over e b i t d a": "Enterprise value over EBITDA"}
        )
...
def financial_ratios(l_args, s_ticker):
    parser = argparse.ArgumentParser(
        prog="ratios",
        description="""
            Prints in-depth ratios of a company over time. This can be either
