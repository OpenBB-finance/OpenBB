...
def overview(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='overview',
                                     description="""Prints an overview about the company. The following fields are
                                     expected:
                                     Symbol, Asset type, Name, Description, Exchange, Currency, Country, Sector,
                                     Industry,
                                     Address, Full time employees, Fiscal year end, Latest quarter,
                                     ...
                                     Return on equity TTM, Revenue TTM, Gross profit TTM, Diluted EPS TTM,
                                     Quarterly earnings growth YOY,
                                     Quarterly revenue growth YOY, Analyst target price, Trailing PE, Forward PE,
                                     Price to sales ratio TTM, Price to book ratio, EV to revenue, EV to EBITDA,
                                     Beta, 52 week high,
                                     52 week low, 50 day moving average, 200 day moving average, Shares outstanding,
                                     Shares float,
                                     Shares short, Shares short prior month, Short ratio, Short percent outstanding,
                                     Short percent float,
                                     Percent insiders, Percent institutions, Forward annual dividend rate,
                                     Forward annual dividend yield,
                                     Payout ratio, Dividend date, Ex dividend date, Last split factor, and Last split
                                     date.
    ...
    try:
        ...
        s_req = f"https://www.alphavantage.co/query?function=OVERVIEW&symbol={s_ticker}&apikey=" \
                f"{cfg.API_KEY_ALPHAVANTAGE}"
...
def key(l_args, s_ticker):
    ...
    try:
        ...
        s_req = f"https://www.alphavantage.co/query?function=OVERVIEW&symbol={s_ticker}" \
...
def cash_flow(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='cash',
                                     ...
                                     investments,
                                     Sales maturities of investments, Stock based compensation. [Source: Alpha
                                     Vantage]""")
...
def earnings(l_args, s_ticker):
    ...
    try:
        ...
        s_req = f"https://www.alphavantage.co/query?function=EARNINGS&symbol={s_ticker}&apikey=" \
        ...
        if result.status_code == 200:
            ...
            if ns_parser.b_quarter:
                ...
                df_fa = df_fa[["fiscalDateEnding", "reportedDate", "reportedEPS", "estimatedEPS", "surprise",
                               "surprisePercentage"]]
