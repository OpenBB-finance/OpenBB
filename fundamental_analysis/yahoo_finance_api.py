...
def info(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='info',
                                     description="""Print information about the company. The following fields are
                                     expected:
                                     Zip, Sector, Full time employees, Long business summary, City, Phone, State,
                                     Country,
                                     Website, Max age, Address, Industry, Previous close, Regular market open,
                                     Two hundred
                                     day average, Payout ratio, Regular market day high, Average daily volume 10 day,
                                     Regular market previous close, Fifty day average, Open, Average volume 10 days,
                                     Beta,
                                     Regular market day low, Price hint, Currency, Trailing PE, Regular market volume,
                                     Market cap, Average volume, Price to sales trailing 12 months, Day low, Ask,
                                     Ask size,
                                     Volume, Fifty two week high, Forward PE, Fifty two week low, Bid, Tradeable,
                                     Bid size,
                                     Day high, Exchange, Short name, Long name, Exchange timezone name,
                                     Exchange timezone
                                     short name, Is esg populated, Gmt off set milliseconds, Quote type, Symbol,
                                     Message board id,
                                     Market, Enterprise to revenue, Profit margins, Enterprise to ebitda,
                                     52 week change,
                                     Forward EPS, Shares outstanding, Book value, Shares short, Shares percent shares
                                     out,
                                     Last fiscal year end, Held percent institutions, Net income to common,
                                     Trailing EPS,
    ...
    try:
        ...
        df_info.index = [''.join(' ' + char if char.isupper() else char.strip() for char in idx).strip() for idx in
...
def shareholders(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='shrs',
                                     description="""Print Major, institutional and mutualfunds shareholders. [Source:
                                     Yahoo Finance]""")
    ...
    try:
        ...
        df_institutional_shareholders['Shares'] = df_institutional_shareholders['Shares'].apply(
            lambda x: long_number_format(x))
        df_institutional_shareholders['Value'] = df_institutional_shareholders['Value'].apply(
            lambda x: long_number_format(x))
        df_institutional_shareholders['Stake'] = df_institutional_shareholders['Stake'].apply(
...
def sustainability(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='sust',
                                     description="""Print sustainability values of the company. The following fields
                                     are expected:
                                     Palmoil, Controversialweapons, Gambling, Socialscore, Nuclear, Furleather,
                                     Alcoholic, Gmo,
                                     Catholic, Socialpercentile, Peercount, Governancescore, Environmentpercentile,
                                     Animaltesting,
                                     Tobacco, Totalesg, Highestcontroversy, Esgperformance, Coal, Pesticides, Adult,
                                     Percentile,
                                     Peergroup, Smallarms, Environmentscore, Governancepercentile, Militarycontract.
    ...
    try:
        ...
        df_sustainability.index = [''.join(' ' + char if char.isupper() else char.strip() for char in idx).strip() for
                                   idx in df_sustainability.index.tolist()]
...
def calendar_earnings(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='calendar_earnings',
                                     description="""Calendar earnings of the company. Including revenue and earnings
                                     estimates.
                                     [Source: Yahoo Finance]""")
    ...
    try:
        ...
        print(
            ...
            f"{df_calendar.iloc[:, 0]['Earnings Low']}, {df_calendar.iloc[:, 0]['Earnings High']}]")
        print(
