...
def management(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='mgmt',
                                     description="""Print management team. Namely: Name, Title, Information from
                                     google and
    ...
    try:
        ...
        for s_name in df_management.index:
            df_management.loc[s_name]['Info'] = f"http://www.google.com/search?q={s_name} {s_ticker.upper()}".replace(
                ' ', '%20')
        ...
        for ind in df_management.index:
            ...
            s_title = f"{df_management['Title'][ind]}" \
                      f"{(max([len(x) for x in df_management['Title']]) - len(df_management['Title'][ind])) * ' '}"
