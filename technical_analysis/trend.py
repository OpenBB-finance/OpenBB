...
def adx(l_args, s_ticker, s_interval, df_stock):
    ...
    if s_interval == "1440min":  # Daily
        _plot_adx(ns_parser, title, df_stock, '5. adjusted close')
    else:  # Intraday
        _plot_adx(ns_parser, title, df_stock, '4. close')
    print("")


def _plot_adx(ns_parser, title, df_stock, data_field):
    df_ta = ta.adx(high=df_stock['2. high'], low=df_stock['3. low'], close=df_stock[data_field],
