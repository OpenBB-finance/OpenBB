...
def cci(l_args, s_ticker, s_interval, df_stock):
    ...
    if s_interval == "1440min":  # Daily
        _plot_cci(ns_parser, '5. adjusted close', df_stock, title)
    else:  # Intraday
        _plot_cci(ns_parser, '4. close', df_stock, title)
    print("")


def _plot_cci(ns_parser, data_field, df_stock, title):
    df_ta = ta.cci(high=df_stock['2. high'], low=df_stock['3. low'], close=df_stock[data_field],
                   length=ns_parser.n_length, scalar=ns_parser.n_scalar, offset=ns_parser.n_offset).dropna()
    plt.subplot(211)
    plt.title(title)
    plt.plot(df_stock.index, df_stock[data_field].values, 'k', lw=2)
...
def macd(l_args, s_ticker, s_interval, df_stock):
    ...
    if s_interval == "1440min":  # Daily
        _plot_macd(ns_parser, '5. adjusted close', df_stock, title)
    else:  # Intraday
        _plot_macd(ns_parser, '4. close', df_stock, title)
    print("")


def _plot_macd(ns_parser, data_field, df_stock, title):
    df_ta = ta.macd(df_stock[data_field], fast=ns_parser.n_fast, slow=ns_parser.n_slow,
...
def rsi(l_args, s_ticker, s_interval, df_stock):
    ...
    if s_interval == "1440min":  # Daily
        _plot_momentum(ns_parser, df_stock, s_ticker, '5. adjusted close', title)
    else:  # Intraday
        _plot_momentum(ns_parser, df_stock, s_ticker, '4. close', title)
    print("")


def _plot_momentum(ns_parser, df_stock, s_ticker, field_name, title):
    df_ta = ta.rsi(df_stock[field_name], length=ns_parser.n_length, scalar=ns_parser.n_scalar,
                   drift=ns_parser.n_drift, offset=ns_parser.n_offset).dropna()
    plt.subplot(211)
    plt.plot(df_stock.index, df_stock[field_name].values, 'k', lw=2)
    plt.title(title)
    plt.xlim(df_stock.index[0], df_stock.index[-1])
    plt.ylabel(f'Share Price ($)')
    plt.grid(b=True, which='major', color='#666666', linestyle='-')
...
def stoch(l_args, s_ticker, s_interval, df_stock):
    ...
    if s_interval == "1440min":  # Daily
        _plot_stoch(ns_parser, df_stock, '5. adjusted close', title)
    else:  # Intraday
        _plot_stoch(ns_parser, df_stock, '4. close', title)
    print("")


def _plot_stoch(ns_parser, df_stock, data_field, title):
    df_ta = ta.stoch(high=df_stock['2. high'], low=df_stock['3. low'], close=df_stock[data_field],
