...
def cci(l_args, s_ticker, s_interval, df_stock):
    ...
    parser.add_argument('-l', "--length", action="store", dest="n_length", type=check_positive, default=14,
                        help='length')
    parser.add_argument('-s', "--scalar", action="store", dest="n_scalar", type=check_positive, default=0.015,
                        help='scalar')
...
def macd(l_args, s_ticker, s_interval, df_stock):
    parser = argparse.ArgumentParser(prog='macd',
                                     ...
                                     current trend, especially if the MACD is at extreme high or low values. When the
                                     MACD
                                     line crosses above the signal line a buy signal is generated. When the MACD
                                     crosses
...
def stoch(l_args, s_ticker, s_interval, df_stock):
    ...
    try:
        ...
        # Daily
        if s_interval == "1440min":
            df_ta = ta.stoch(high=df_stock['2. high'], low=df_stock['3. low'], close=df_stock['5. adjusted close'],
                             k=ns_parser.n_fastkperiod,
                             d=ns_parser.n_slowdperiod, smooth_k=ns_parser.n_slowkperiod,
        ...
        # Intraday
            ...
            df_ta = ta.stoch(high=df_stock['2. high'], low=df_stock['3. low'], close=df_stock['4. close'],
                             k=ns_parser.n_fastkperiod,
                             d=ns_parser.n_slowdperiod, smooth_k=ns_parser.n_slowkperiod,
                             offset=ns_parser.n_offset).dropna()
