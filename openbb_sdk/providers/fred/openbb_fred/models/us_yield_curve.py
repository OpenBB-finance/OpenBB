...
from openbb_fred.utils.fred_helpers import YIELD_CURVE_NOMINAL_RATES, YIELD_CURVE_REAL_RATES, YIELD_CURVE_SERIES_REAL, YIELD_CURVE_SERIES_NOMINAL
from openbb_provider.abstract.fetcher import Fetcher
from openbb_provider.standard_models.us_yield_curve import USYieldCurveData, USYieldCurveQueryParams
...
class FREDYieldCurveFetcher(Fetcher[FREDYieldCurveQueryParams, FREDYieldCurveData]):
    ...
    @staticmethod
    def extract_data(
        query: FREDYieldCurveQueryParams, credentials: Optional[Dict[str, str]], **kwargs: Any
    ) -> dict:
        ...
        for series in fred_series.values():
            data = fred.get_series(
                series, start_date=start_date, **kwargs
            )

            if date:
                # if date is not empty, loop through the data and find the corresponding value
                value = sorted(data, key=lambda x: abs(datetime.strptime(x['date'],"%Y-%m-%d").date() - date))[0]['value']
            else:
    ...
    @staticmethod
    def transform_data(data: dict) -> List[Dict[str, List[FREDYieldCurveData]]]:
