...
def regression(l_args, s_ticker, s_interval, df_stock, polynomial):
    ...
    if polynomial == USER_INPUT:
        parser.add_argument('-p', "--polynomial", action="store", dest="n_polynomial", type=check_positive,
                            required=True,
    ...
    try:
        ...
        # Split training data
        stock_x, stock_y = splitTrain.split_train(df_stock['5. adjusted close'].values, ns_parser.n_inputs,
        ...
        if polynomial == LINEAR:
            ...
            model = pipeline.make_pipeline(preprocessing.PolynomialFeatures(polynomial), linear_model.Ridge())

        model.fit(stock_x, stock_y)
        l_predictions = model.predict(df_stock['5. adjusted close'].values[-ns_parser.n_inputs:].reshape(1, -1))[0]
