...
def fbprophet(l_args, s_ticker, s_interval, df_stock):
    parser = argparse.ArgumentParser(prog='fbprophet',
                                     description="""Facebook Prophet is a forecasting procedure that is fast and
                                     provides
                                     completely automated forecasts that can be tuned by hand by data scientists and
                                     analysts.
    ...
    try:
        ...
        l_pred_days = get_next_stock_market_days(last_stock_day=pd.to_datetime(df_stock['ds'].values[-1]),
