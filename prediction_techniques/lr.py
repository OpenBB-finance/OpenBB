...
# ----------------------------------------------------- LR -----------------------------------------------------
def linear_regression(l_args, s_ticker, s_interval, df_stock):
    parser = argparse.ArgumentParser(prog='linear_regression',
                                     description="""Linear regression attempts to model the relationship between 
    ...
    try:
        ...
        lr.fit(stock_x, stock_y)
        
        # Prediction data
        l_predictions = lr.predict(df_stock['4. close'].values[-ns_parser.n_inputs:].reshape(1, -1))[0]
        ...
        plt.title(f"Linear Regression on {s_ticker} - {ns_parser.n_days} days prediction")
