...
try:
    _all_tags = []
    _biztoc_tags = (
        get_all_tags(api_key=obb.user.credentials.biztoc_api_key.get_secret_value())  # type: ignore
    )
...
try:
    _sources = (
        get_sources(api_key=obb.user.credentials.biztoc_api_key.get_secret_value())  # type: ignore
    )
...
if "selected_provider" not in st.session_state:
    if len(news_sources) == 0:
        st.error(f"No news sources available. Please check your credentials for one of: {SUPPORTED_SOURCES}")
        st.stop()
    if len(news_sources) > 0:
        st.session_state.selected_provider = "Biztoc" if "Biztoc" in news_sources else news_sources[0]
...
def fetch_openbb():
    ...
    if st.session_state.selected_provider == "Benzinga":
        ...
        kwargs["channels"] = (
            ...
            if st.session_state.selected_benzinga_channel else None
        )
        kwargs["symbol"] = (
            st.session_state.benzinga_tickers
            if st.session_state.benzinga_tickers else None
        )
    ...
    if st.session_state.selected_provider == "FMP":
        kwargs["symbol"] = st.session_state.fmp_tickers if st.session_state.fmp_tickers else None

    if st.session_state.selected_provider == "Intrinio":
        kwargs["symbol"] = st.session_state.intrinio_tickers if st.session_state.intrinio_tickers else None

    if st.session_state.selected_provider == "Tiingo":
        kwargs["start_date"] = st.session_state.news_start_date.strftime("%Y-%m-%d")
        kwargs["end_date"] = st.session_state.news_end_date.strftime("%Y-%m-%d")
        kwargs["symbol"] = st.session_state.tiingo_tickers if st.session_state.tiingo_tickers else None
...
with st.sidebar:
    ...
    with c1:
        ...
        st.session_state.selected_provider = st.selectbox(
            label="Provider", options=news_sources, index=news_sources.index(st.session_state.selected_provider)
        )
        old_tags = st.session_state.selected_tags
        if st.session_state.selected_provider == "Benzinga":
            st.session_state.news_start_date = st.date_input("Start Date", value=old_start_date)
            st.session_state.selected_tags = st.text_input(label="Tag", value=st.session_state.selected_tags)
        old_biztoc_tags = st.session_state.biztoc_tags
        old_biztoc_source = st.session_state.selected_biztoc_source
        if st.session_state.selected_provider == "Biztoc":
            st.session_state.selected_tags = st.selectbox(label="Tag", options=[None]+old_biztoc_tags)
            st.session_state.selected_biztoc_source = (
                st.selectbox(label="Source", options=[None] + st.session_state.biztoc_sources)
            )
        old_benzinga_tickers = st.session_state.benzinga_tickers
        if st.session_state.selected_provider == "Benzinga":
            st.session_state.benzinga_tickers = st.text_input(label="Tickers", value=old_benzinga_tickers)
        old_fmp_tickers = st.session_state.fmp_tickers
        if st.session_state.selected_provider == "FMP":
            st.session_state.fmp_tickers = st.text_input(label="Tickers", value=old_fmp_tickers)
        old_intrinio_tickers = st.session_state.intrinio_tickers
        if st.session_state.selected_provider == "Intrinio":
            st.session_state.intrinio_tickers = st.text_input(label="Tickers", value=old_intrinio_tickers)
        old_tiingo_tickers = st.session_state.tiingo_tickers
        if st.session_state.selected_provider == "Tiingo":
            st.session_state.news_start_date = st.date_input("Start Date", value=old_start_date)
            old_tiingo_tickers = st.session_state.tiingo_tickers
            st.session_state.tiingo_tickers = st.text_input(label="Tickers", value=st.session_state.tiingo_tickers)
    with c2:
        old_limit = st.session_state.selected_limit
        old_end_date = st.session_state.news_end_date
        st.session_state.selected_limit = st.number_input("Number of Stories", min_value=1, value=100)
        old_channel = st.session_state.selected_benzinga_channel
        if st.session_state.selected_provider == "Benzinga":
            st.session_state.news_end_date = st.date_input("End Date", value=old_end_date)
            st.session_state.selected_benzinga_channel = st.text_input(label="Feed Channel", value=old_channel)
        old_term = st.session_state.selected_term
        if st.session_state.selected_provider == "Biztoc":
            st.session_state.selected_term = st.text_input(label="Search Term", value=old_term)
        if st.session_state.selected_provider == "Tiingo":
            st.session_state.news_end_date = st.date_input("End Date", value=old_end_date)
    ...
    with st.session_state.news_container.container():
        ...
        if st.session_state.news is not None:
            ...
            for i in st.session_state.news.index:
                ...
                expanded = story == 0
                text = (
                    st.session_state.news.loc[i].text if "text" in st.session_state.news.loc[i]
                ...
                if text != "":
                    with st.expander(
                        ...
                        if st.session_state.selected_provider == "Benzinga":
                            _tags = (
                                st.session_state.news.loc[i].tags if st.session_state.news.loc[i].get("tags") else ""
                            )
                            _stocks = (
                                st.session_state.news.loc[i].stocks if st.session_state.news.loc[i].get("stocks") else ""
                            )
                            _channels = (
                                st.session_state.news.loc[i].channels
                                if st.session_state.news.loc[i].get("channels") else ""
                            )
                            _images = (
                                st.session_state.news.loc[i].images if st.session_state.news.loc[i].get("images") else []
                            )
                            ...
                            st.write(_url)
                            if _tags:
                                st.markdown(f"##### Tags for this story:  \n {_tags}  \n")
                            if _stocks:
                                st.markdown(f"##### Stocks mentioned:\n {_stocks}  \n")
                            if _channels:
                                st.markdown(f"##### Channels for this story:  \n {_channels}  \n")

                        if st.session_state.selected_provider == "Biztoc":
                            if st.session_state.news.loc[i].get("images"):
                                img = st.session_state.news.loc[i].images.get("s")
                                img = st.session_state.news.loc[i].images.get("o") if img is None else img
                                if img is not None:
                                    st.markdown(
                                        f"<div style='text-align: center;'><img src='{img}'></div><br></br>",
                                        unsafe_allow_html=True,
                        ...
                        if st.session_state.selected_provider == "Intrinio":
                            ...
                            _stocks = (
                                ...
                                if st.session_state.news.loc[i].get("company") else None
                            )
            ...
            st.divider()
        st.write("Learn more about the OpenBB Platform [here](https://docs.openbb.co/platform)")
