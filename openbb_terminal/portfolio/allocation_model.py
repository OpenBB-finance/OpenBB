...
def obtain_assets_allocation(benchmark_info: Dict, trade_value):
    ...
    portfolio_assets_allocation = (
        (
            trade_value[trade_value["Type"] != "CASH"]
            .groupby(by="Name")
            .agg({"Portfolio Value": "sum"})
            .div(trade_value["Portfolio Value"].sum())
        )
        .squeeze()
...
@log_start_end(log=logger)
def obtain_sector_allocation(benchmark_info: Dict, trade_value):
...
def obtain_benchmark_regional_and_country_allocation(
    benchmark_ticker: str, regional_test=None, country_test=None
):
...
@log_start_end(log=logger)
def obtain_portfolio_regional_and_country_allocation(trade_value):
    ...
    Returns
    ------
    portfolio_regional_allocation: dict
        Dictionary with regional allocations.
    portfolio_country_allocation: dict
        Dictionary with country allocations
    """
