...
function autoScaling(eventdata, graphs) {
    try {
        if (eventdata['xaxis.range[0]'] != undefined) {
            let log_scale = true ? graphs.layout.yaxis != undefined && graphs.layout.yaxis.type == 'log' : false;

            let secondary_y = true
                ? graphs.layout.yaxis2 != undefined && graphs.layout.yaxis2.overlaying == 'y'
                : false;
            ...
            for (let trace of graphs.data) {
                let yaxis = !trace.yaxis ? 'yaxis' : 'yaxis' + trace.yaxis.replace('y', '');
...
function downloadData(gd) {
    ...
    let xaxis =
        'title' in gd.layout['xaxis'] && gd.layout['xaxis']['title']['text'] != undefined
            ? gd.layout['xaxis']['title']['text']
            : 'x';

    let yaxis =
        'title' in gd.layout['yaxis'] && gd.layout['yaxis']['title']['text'] != undefined
