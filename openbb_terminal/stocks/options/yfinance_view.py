...
def show_parity(
    ...
    if div_dts:
        ...
        avg_div = np.mean(div_info.to_list()[-4:]) if len(div_dts) > 3 else np.mean(div_info.to_list())
...
def risk_neutral_vals(
    ...
    """
    chain = get_option_chain(symbol, expiry).puts if put else get_option_chain(symbol, expiry).calls
