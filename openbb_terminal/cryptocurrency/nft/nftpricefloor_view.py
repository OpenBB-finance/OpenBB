...
def display_collections(show_fp: bool = False, show_sales: bool = False, num: int = 5, export: str = ""):
        ...
        if show_fp or show_sales:
            ...
            for collection in df['slug'].head(num).values:
                ...
                if not df_collection.empty:
                    values = df_collection['dataPriceFloorETH'] if show_fp else df_collection['sales']
...
def display_floor_price(
    ...
    if df.empty:
        ...
        if external_axes is None:
            ...
            (ax,) = external_axes
        ax.bar(df.index, df["sales"], color=theme.down_color)
        ax.set_xlim(
            df.index[0],
            df.index[-1],
        )
        ...
        ax2 = ax.twinx()
        ax2.plot(df["dataPriceFloorETH"], color=theme.up_color)
