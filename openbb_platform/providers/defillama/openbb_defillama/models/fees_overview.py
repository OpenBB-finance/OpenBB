...
class DeFiLlamaFeesOverviewData(Data):
    """DeFiLlama Fees Overview Data."""
    ...
    breakdown_24h: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Total fees collected in the last 24 hours broken down by chains",
        alias="breakdown24h",
    )
    total_24h: int = Field(description="Total 24h fees.", alias="total24h")
    total_48h_to_24h: int = Field(
        description="Total 48h to 24h fees.", alias="total48hto24h"
    )
    total_7d: int = Field(description="Total 7d fees.", alias="total7d")
    total_14d_to_7d: int = Field(
        description="Total 14d to 7d fees.", alias="total14dto7d"
    )
    total_60d_to_30d: int = Field(
        description="Total 60d to 30d fees.", alias="total60dto30d"
    )
    total_30d: int = Field(description="Total 30d fees.", alias="total30d")
    total_1y: int = Field(description="Total 1y fees.", alias="total1y")
    change_1d: float = Field(description="1d change in fees.")
    change_7d: float = Field(description="7d change in fees.")
    change_1m: float = Field(description="1m change in fees.")
    change_7d_over_7d: float = Field(
        description="7d change in fees over 7d.", alias="change_7dover7d"
    )
    change_30d_over_30d: float = Field(
        description="30d change in fees over 30d.", alias="change_30dover30d"
    )
...
class DeFiLlamaFeesOverviewFetcher(
    Fetcher[DeFiLlamaFeesOverviewQueryParams, List[DeFiLlamaFeesOverviewData]]
):
    """DeFiLlama Fees Overview Fetcher."""
    ...
    async def aextract_data(
        query: DeFiLlamaFeesOverviewQueryParams,
    ...
    @staticmethod
    def transform_data(
        query: DeFiLlamaFeesOverviewQueryParams, data: Dict[str, Any], **kwargs: Any
    ) -> List[DeFiLlamaFeesOverviewData]:
        ...
        return DeFiLlamaFeesOverviewData.model_validate(transformed_data)
