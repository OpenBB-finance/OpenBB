...
# BIS does not play well with newer python.  This code block from stackoverflow helps
...
def fetch_data(url: str, csv_kwargs: Optional[Dict] = None, **kwargs: Any) -> DataFrame:
    """Create a session and fetch data from the BIS API."""
...
### The functions below are for using the new bis data-explorer instead of the stats.bis


def bis_xml_to_df(xml_string: str) -> DataFrame:
    """Parse the BIS XML and return a dataframe.

    Parameters
    ----------
    xml_string : str
        A string containing the BIS XML data.
...
def parse_url(url: str) -> DataFrame:
    ...
    return bis_xml_to_df(response.text)
...
def bis_date_to_python_date(input_date: Union[str, int]) -> date:
