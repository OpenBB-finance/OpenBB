...
class FredEconomicCalendarQueryParams(UseCacheQueryParams, EconomicCalendarQueryParams):
...
class FredEconomicCalendarFetcher(
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        from openbb_fred.utils.rate_limiter import fred_get, fred_get_many
        ...
        async def fetch_date_pages(
            ...
            first_page = await fred_get(
                base_url,
                response_callback=response_callback,
                headers=request_headers(),
                use_cache=query.use_cache,
            )
            n_results = first_page.get("ptic", 0)

            if n_results == 0:
                return DataFrame()

            records = [process_pager(first_page["pager"], date=start_date)]

            if len(records[0]) >= n_results:
                return records[0]

            total_pages = ceil(n_results / 50)
            responses = await fred_get_many(
            ...
            for resp in responses:
                records.append(process_pager(resp["pager"], date=start_date))
