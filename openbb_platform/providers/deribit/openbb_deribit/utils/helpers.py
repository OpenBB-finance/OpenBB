...
def bracketing_expirations(instruments: list[dict], target: int) -> set:
    """Return the two expirations either side of a moment.

    Parameters
    ----------
    instruments : list[dict]
        The option instruments of one currency.
    target : int
        The moment to bracket, in milliseconds since the epoch.

    Returns
    -------
    set
        The expiration timestamps immediately below and above the target.
    """
    expirations = sorted({d["expiration_timestamp"] for d in instruments})
    below = [e for e in expirations if e <= target]
    above = [e for e in expirations if e > target]

    return {*below[-1:], *above[:1]}


async def get_volatility_index_options(currency: str = "BTC") -> list[dict]:
    """Return the options a volatility index is built from.

    The exchange keeps a mark price history only for these, which are the
    contracts on the two expirations bracketing thirty days.

    Parameters
    ----------
    currency : str
        The currency whose volatility index to read.

    Returns
    -------
    list[dict]
        The bracketing contracts, nearest expiration first.
    """
    from datetime import datetime, timedelta, timezone

    listed = await get_instruments(currency.upper(), "option")

    if not listed:
        return []

    target = to_timestamp(datetime.now(timezone.utc) + timedelta(days=30))
    bracket = bracketing_expirations(listed, target)

    return sorted(
        (d for d in listed if d["expiration_timestamp"] in bracket),
        key=lambda d: (d["expiration_timestamp"], d.get("strike") or 0),
    )


async def default_volatility_option(currency: str = "BTC") -> str:
    """Return the contract to show when none was asked for.

    Returns
    -------
    str
        The strike nearest the index on the nearer of the two expirations a
        volatility index is built from.

    Raises
    ------
    EmptyDataError
        If the exchange lists no such contract.
    """
    from openbb_core.provider.utils.errors import EmptyDataError

    from openbb_deribit.utils.client import request

    listed = await get_volatility_index_options(currency)

    if not listed:
        raise EmptyDataError(
            f"Deribit lists no volatility index options on {currency.upper()}."
        )

    nearest = listed[0]["expiration_timestamp"]
    front = [d for d in listed if d["expiration_timestamp"] == nearest]
    index = await request("get_index_price", {"index_name": f"{currency.lower()}_usd"})
    spot = (index or {}).get("index_price") or 0

    return str(
        min(front, key=lambda d: abs((d.get("strike") or 0) - spot))["instrument_name"]
    )
