...
FLOW_CODES = {"total": "TOTIMPSB", "pipeline": "IMPPIP", "lng": "IMPLNG"}
FLOW_LABELS = {
    "total": "Imports",
    "pipeline": "Pipeline imports",
    "lng": "LNG imports",
}


class JodiGasImportsQueryParams(QueryParams):
    """JODI Gas Imports Query.
    ...
    flow: Literal["total", "pipeline", "lng"] = Field(
        default="total",
        description="The imports breakdown. JODI questionnaire definitions:\n"
        + "    total: "
        + GAS_FLOW_DEFINITIONS["imports"]
        + "\n    pipeline: "
        + GAS_FLOW_DEFINITIONS["pipeline_imports"]
        + "\n    lng: "
        + GAS_FLOW_DEFINITIONS["lng_imports"],
    )
...
class JodiGasImportsData(JodiCountryData):
    """JODI Gas Imports Data.

    One column per reporting country. Imports are amounts that have
...
class JodiGasImportsFetcher(
    Fetcher[JodiGasImportsQueryParams, list[JodiGasImportsData]]
):
    """JODI Gas Imports Fetcher."""

    @staticmethod
    def transform_query(params: dict[str, Any]) -> JodiGasImportsQueryParams:
        """Transform the query params."""
        return JodiGasImportsQueryParams(**params)

    @staticmethod
    async def aextract_data(
        query: JodiGasImportsQueryParams,
    ...
    def transform_data(
        ...
        data: list[dict],
        **kwargs: Any,
    ) -> AnnotatedResult[list[JodiGasImportsData]]:
        ...
        return AnnotatedResult(
            result=[JodiGasImportsData.model_validate(r) for r in rows],
            metadata=metadata,
        )
