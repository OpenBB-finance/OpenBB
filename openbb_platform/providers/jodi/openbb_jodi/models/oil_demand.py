...
from openbb_jodi.utils.constants import (
    ...
    OilSecondaryProductType,
    OilUnitType,
)

SECONDARY_PRODUCTS = [OIL_PRODUCT_FIELDS[code] for code in OIL_PRODUCT_FIELDS]


class JodiOilDemandQueryParams(QueryParams):
    """JODI Oil Demand Query.
    ...
    __json_schema_extra__ = {
        ...
        "product": {"choices": SECONDARY_PRODUCTS},
    ...
    product: OilSecondaryProductType = Field(
        default="total_products",
        description="The refined petroleum product to get demand of."
        + " Demand is reported for secondary products only."
    ...
    def _normalize_tokens(cls, v, info):
        ...
        defaults = {"product": "total_products", "unit": "kbd"}
        return normalize_token(v, defaults[str(info.field_name)])


class JodiOilDemandData(JodiCountryData):
    """JODI Oil Demand Data.

    One column per reporting country. Demand is deliveries or sales to
    the inland market (domestic consumption) plus refinery fuel plus
    international marine and aviation bunkers.
    """


class JodiOilDemandFetcher(Fetcher[JodiOilDemandQueryParams, list[JodiOilDemandData]]):
    """JODI Oil Demand Fetcher."""

    @staticmethod
    def transform_query(params: dict[str, Any]) -> JodiOilDemandQueryParams:
        """Transform the query params."""
        return JodiOilDemandQueryParams(**params)

    @staticmethod
    async def aextract_data(
        query: JodiOilDemandQueryParams,
    ...
    def transform_data(
        ...
        data: list[dict],
        **kwargs: Any,
    ) -> AnnotatedResult[list[JodiOilDemandData]]:
        ...
        return AnnotatedResult(
            result=[JodiOilDemandData.model_validate(r) for r in rows],
            metadata=metadata,
        )
