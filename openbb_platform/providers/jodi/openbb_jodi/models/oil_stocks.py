...
class JodiOilStocksQueryParams(QueryParams):
    """JODI Oil Stocks Query.
    ...
    product: OilProductType = Field(
        default="crude_oil",
        description="The petroleum product to get closing stock levels of."
    ...
    def _normalize_tokens(cls, v, info):
        ...
        defaults = {"product": "crude_oil", "unit": "kbbl"}
        return normalize_token(v, defaults[str(info.field_name)])


class JodiOilStocksData(JodiCountryData):
    """JODI Oil Stocks Data.

    One column per reporting country. Closing stocks are the primary
    stock level at the end of the month within national territories,
    including stocks held by importers, refiners, stock-holding
    organisations, and governments.
    """


class JodiOilStocksFetcher(Fetcher[JodiOilStocksQueryParams, list[JodiOilStocksData]]):
    """JODI Oil Stocks Fetcher."""

    @staticmethod
    def transform_query(params: dict[str, Any]) -> JodiOilStocksQueryParams:
        """Transform the query params."""
        return JodiOilStocksQueryParams(**params)

    @staticmethod
    async def aextract_data(
        query: JodiOilStocksQueryParams,
    ...
    def transform_data(
        ...
        data: list[dict],
        **kwargs: Any,
    ) -> AnnotatedResult[list[JodiOilStocksData]]:
        ...
        return AnnotatedResult(
            result=[JodiOilStocksData.model_validate(r) for r in rows],
            metadata=metadata,
        )
