...
@router.command(
    methods=["GET"],
    widget_config={
        "name": "IMF Indicators by Dataflow",
        "params": [
            {
                "paramName": "dataflow_id",
                "label": "Dataflow ID",
                "value": "MFS_IR",
                "description": "The IMF dataflow ID to list indicators for.",
                "type": "endpoint",
...
async def list_indicators_by_dataflow(
...
) -> OBBject[list[ImfIndicatorMetadata]]:
    """List all indicator-like dimensions for a specified IMF dataflow."""
    metadata = ImfMetadata()
    try:
        indicators = metadata.get_indicators_in(dataflow_id)
    ...
    return OBBject(
        results=[ImfIndicatorMetadata(**indicator) for indicator in indicators]
    )
