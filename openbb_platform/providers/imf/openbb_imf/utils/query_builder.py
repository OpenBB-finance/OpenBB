    ...
    def build_url(
        ...
        **kwargs,
        ...
        def format_date(
            ...
            if not date_str:  # pragma: no cover -- unreachable: every caller wraps the call in ``if start_date:`` / ``if end_date:`` so an empty value never reaches this branch
                return date_str
    ...
    def fetch_data(
        ...
        **kwargs
            ...
            for comp in group.findall("Comp") + group.findall("ss:Comp", namespaces):
                ...
                if comp_id:
                    ...
                    if value_elem is None:
                        value_elem = comp.find("ss:Value", namespaces)
        ...
        for series in series_elements:
            ...
            elif indicator_code:  # pragma: no cover -- unreachable: the surrounding loop always populates ``indicator_codes_list`` whenever ``indicator_code`` is set
                ...
                if attr_value is not None and attr_value != [None]:
                    if attr_id == "TOPIC_DATASET":
                        ...
                        if date_value:
                            ...
                            except (  # pragma: no cover -- unreachable: defensive guard around ``str.split(".")`` which never raises on any string value
                                Exception
                            ):
                                dataset_attrs["last_updated"] = date_value
