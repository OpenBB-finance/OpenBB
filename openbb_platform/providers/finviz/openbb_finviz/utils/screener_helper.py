...
def get_screener_data(
    ...
    if sort_by is not None and sort_by not in d_check_screener["Order"]:
        raise ValueError(f"Sort value provided must be one of: {d_check_screener['Order']}")

    if preset in d_signals:
        if preset == "top_losers":
            ascend=True
        screen.set_filter(signal=d_signals[preset])
        ...
        try:
            df_screen = screen.screener_view(
                ...
                ascend=ascend,
                sleep_sec=sleep,
                order=sort_by if sort_by is not None else "Change",
                verbose=False
            )
        except (IndexError, AttributeError):
            raise ValueError("Invalid data provided by the website.")
        ...
        df_screen = screen.screener_view(
            order=sort_by if sort_by is not None else d_general.get("Order", "Change"),
            limit=limit if limit else 100000,
            ascend=ascend,
            sleep_sec=sleep,
            verbose=False,
