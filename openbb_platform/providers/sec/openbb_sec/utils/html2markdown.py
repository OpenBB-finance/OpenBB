...
# flake8: noqa: PLR0911, PLR0912, PLR0913, PLR0914, PLR0915, PLR0916, PLR0917, R1702
...
def html_to_markdown(
    ...
    def _count_data_columns(table):
        ...
        for row in rows[:5]:
            ...
            for cell in cells:
                text = (
                    cell.get_text().replace("\u200b", "").replace("\xa0", " ").strip()
                )
                if text:
    ...
    # Use module-level image converter with base_url bound
    def convert_image_to_markdown(img):
        """Local wrapper for _convert_image_to_html with base_url from outer scope."""
        return _convert_image_to_html(img, base_url)

    def get_text_content(element, preserve_links_in_text: bool = False) -> str:
    ...
    def convert_table(table) -> str:
        ...
        # Helper to strip whitespace and invisible characters
        def strip_all(s):
        ...
        data = []
        raw_rows_with_colspan = (
            []
        )  # For header detection - stores (text, colspan) pairs
        ...
        if len(data) <= 2:
            ...
            if len(non_empty) == 2:
                ...
                if (
                    first_cell.isdigit()
                    and any(kw in second_lower for kw in form_keywords)
                ) or (
                    second_cell.isdigit()
                    and any(kw in first_lower for kw in form_keywords)
                ):
                ...
                while i < len(row):
                    ...
                    if i + 1 < len(row):
                        ...
                        if cell_stripped.endswith("(") and next_cell == ")":
                            ...
                            merged_row.append(
                                ""
                            )  # Placeholder to maintain column count
                            i += 2
                            continue
        ...
        def remove_empty_columns(rows):
            ...
            result = [
                [row[i] if i < len(row) else "" for i in cols_to_keep] for row in rows
            ]
            return result
        ...
        def collapse_repeated_headers(rows):
            ...
            for i in range(1, len(header)):
                ...
                if prev and (curr == prev or not curr):
                    ...
                    curr_has_data = any(
                        i < len(row) and row[i].strip() for row in rows[1:]
                    )
            ...
            for col_idx in range(num_cols - 1, -1, -1):
                header_empty = not (
                    header[col_idx].strip() if col_idx < len(header) else ""
                )
                data_empty = all(
                    not (row[col_idx].strip() if col_idx < len(row) else "")
                    for row in rows[1:]
                )
        ...
        def build_column_headers_from_colspan(rows_with_colspan):
            ...
            def is_header_row(parsed):
                ...
                non_empty = [
                    (t, c, s) for t, c, s in parsed if t and has_visible_content(t)
                ]
                if not non_empty:
                ...
                has_data = False
                has_label = (
                    False  # Line item label (text in col 0/1 with small colspan)
                )
                ...
                for t, colspan, start in non_empty:
                    ...
                    if is_year(t):
                        ...
                        rf"(months?|weeks?|quarters?|years?|ended|{MONTHS_PATTERN})",
                        t,
                        re.I,
                    ):
                        # Row labels like "AT DECEMBER 31:" in col 0 shouldn't trigger year detection
                        has_year = True
            ...
            for row_idx, parsed in enumerate(parsed_rows):
                ...
                has_year = any(
                    ...
                    or (
                        ...
                        and colspan <= 2
                        and s
                        > 0  # Skip first column (row labels like "AT DECEMBER 31:")
                    )
                    or (
            ...
            if (
                column_headers_positions
                and column_headers_positions[0] == 0
                and category_rows
            ):
            ...
            if len(year_rows) >= 2:
                ...
                for i in range(len(year_rows)):
                    for j in range(i + 1, len(year_rows)):
                        ...
                        if (a_has_months and b_has_years) or (
                            a_has_years and b_has_months
                        ):
                            result, result_positions = _try_position_merge(
                                hdrs_a, hdrs_b
                            )
                            if result:
            ...
            if not column_headers_list:
                ...
                if len(category_rows) >= 2:
                    ...
                    first_ne = [
                        (t, c, s) for t, c, s in first_parsed if has_visible_text(t)
                    ]
                    is_date_super = False
                    super_text = ""
                    if len(first_ne) == 1:
                    ...
                    if is_date_super:
                        ...
                        if pos_texts:
                            ...
                            merged_sub_headers = [
                                " ".join(pos_texts[p]) for p in sorted_pos
                            ]
                ...
                if category_rows:
                    ...
                    if pos_map:
                        ...
                        if title_period_texts:
                            super_row = ["", title_period_texts[-1]] + [""] * (
                                n_cols - 1
                            )
            ...
            if year_super_headers and len(header_info) >= 2:
                # Check if next row has sub-headers
                sub_header_row = None
                year_row_indices = {idx for idx, _ in year_rows}

                for idx, parsed, is_yr, is_cat in header_info:
                    if idx not in year_row_indices and not is_yr:
                        sub_header_row = (idx, parsed)

                if sub_header_row:
                    _, sub_parsed = sub_header_row
                    sub_headers = []
                    first_col_label = ""

                    for text, colspan, start in sub_parsed:
                        if start == 0 and text:
                            first_col_label = text
                            continue
                    ...
                    if sub_headers:
                        num_sub_headers = len(sub_headers)
                        num_years = len(year_super_headers)
                        subs_per_year = (
                            num_sub_headers // num_years
                            if num_years
                            else num_sub_headers
                        )

                        # Build year row: year appears only in first cell of its group
                        year_row = [first_col_label]

                        for year_text, _, _ in year_super_headers:
                            year_row.append(year_text)  # Year in first position
                            for _ in range(subs_per_year - 1):
            ...
            if (
                column_headers_positions
                and len(column_headers_positions) == num_headers
            ):
        ...
        def extract_periods_from_rows(rows_with_cs, row_has_th_flags=None):
            ...
            def merge_vertical_headers(rows_with_cs, total_cols, row_has_th_flags=None):
                ...
                for row_idx, row in enumerate(rows_with_cs):
                    ...
                    # it's definitely a data row, not a header
                    if (
                        row_has_th_flags
                        and row_idx < len(row_has_th_flags)
                        and not row_has_th_flags[row_idx]
                    ):
                        # Row has only <td> elements - it's a data row
                        break
                ...
                for row_idx, row in header_rows:
                    ...
                    for text, colspan in row:
                        ...
                        if is_header_text(t):
                            # Group by starting position (or close positions for alignment tolerance)
                            # Find closest existing position within 1 column
                            matched_pos = None
                            for existing_pos in position_texts:
                                if abs(existing_pos - col_pos) <= 1:
                                    matched_pos = existing_pos
                                    break

                            if matched_pos is not None:
                                position_texts[matched_pos].append((t, colspan))
                            else:
                                position_texts[col_pos] = [(t, colspan)]
                                position_colspan[col_pos] = colspan
                        col_pos += colspan

                # Need at least 2 column positions for a proper table
                if len(position_texts) < 2:
                    return None, 0

                # Check for multi-level headers: if colspans at same position differ significantly
                # (e.g., parent has 7, child has 3), this is multi-level, not vertical merge
                has_multi_level = False
                for pos, entries in position_texts.items():
                    if len(entries) >= 2:
                        colspans = [cs for _, cs in entries]
                        max_cs = max(colspans)
                        min_cs = min(colspans)
                        # If max colspan is more than 1.5x min, likely multi-level headers
                        if max_cs > 2 and min_cs >= 2 and max_cs > min_cs * 1.5:
                            has_multi_level = True
                            break

                # If multi-level headers detected, don't merge - let build_column_headers_from_colspan handle it
                if has_multi_level:
                    return None, 0

                # Merge texts at same positions vertically
                merged_headers = []
                for pos in sorted(position_texts.keys()):
                    entries = position_texts[pos]
                    texts = [t for t, cs in entries]  # Extract just the text
                    merged = " ".join(texts)

                    # Skip position 0 if it looks like a period description rather than a column header
                    # This happens in tables like OCI where row 3 has "Three Months Ended..." in col 0
                    # alongside actual column headers like "Amount", "Credit" in other columns
                    if pos == 0:
                        # Check if this looks like a period/date description
                        period_patterns = [
                            r"months?\s+ended",  # "Three Months Ended"
                            r"quarter\s+ended",  # "Quarter Ended"
                            r"year\s+ended",  # "Year Ended"
                            r"fiscal\s+\d{4}",  # "Fiscal 2024"
                            r"as\s+of\s+",  # "As of January"
                        ]
                        is_period_label = any(
                            re.search(pat, merged.lower()) for pat in period_patterns
                        )

                        # Also check if position 0 has fewer entries than other positions
                        # (indicating it's not really a vertically merged header)
                        other_max_entries = (
                            max(
                                len(position_texts[p])
                                for p in position_texts  # pylint: disable=C0206
                                if p != pos
                            )
                            if len(position_texts) > 1
                            else 0
                        )
                        is_sparse = len(entries) < other_max_entries

                        if is_period_label or (is_sparse and len(entries) == 1):
                            # Skip this - it's a period label, not a column header
                            continue

                    merged_headers.append(merged)

                # Return as single header layer
            ...
            # Also try vertical header merging (for multi-row stacked headers)
            # This handles cases where headers are stacked vertically:
            #   Before | Tax | After
            #   Tax | (Expense) | Tax
            #   Amount | Credit | Amount
            # Should merge to: "Before Tax Amount" | "Tax (Expense) Credit" | "After Tax Amount"
            ...
            if use_vertical:
                ...
                all_header_text = " ".join(
                    h for layer in vertical_headers for h in layer
                )
                has_year_in_headers = bool(
                    re.search(r"\b(19|20)\d{2}\b", all_header_text)
                )
                ...
                has_year_after_headers = (
                    any(
                        re.search(r"\b(19|20)\d{2}\b", text)
                        for row in rows_with_cs[
                            vertical_row_count : vertical_row_count + 2
                        ]
                        for text, _ in row
                    )
            ...
            for row_idx, row in enumerate(rows_with_cs[:10]):
                ...
                ended_cell_matches = [
                    t for t in texts if re.match(r"^Ended\b", t, re.I)
                ]
                ...
                if ended_cell_matches and period_parts:
                    ...
                    if len(ended_parts) == len(period_parts):
                        ...
                        period_prefixes = [
                            f"{p} {ended_parts[0]}" for p in period_parts
                        ]
                ...
                date_pattern = (
                    rf"^(As\s+of\s+)?({MONTHS_PATTERN})\s+((?:19|20)\d{{2}})$"
                )
                ...
                sub_header_pattern = (
                    r"^(Assets|Liabilities|Actual|Pro\s*Forma|Adjustments)$"
                )
                sub_matches = [
                    t for t in texts if re.match(sub_header_pattern, t, re.I)
                ]
                ...
                if row_idx < 5 and not generic_headers:
                    ...
                    if len(candidates) >= 2:
                        ...
                        alpha_count = sum(
                            1 for c in candidates if re.match(r"^[A-Za-z]", c)
                        )
                        if alpha_count >= 2:
        ...
        def parse_row_semantic(row, num_expected_cols=0):
            ...
            if values and not label:
                ...
                has_currency = any(
                    v.startswith("$") or re.match(r"^\(?\d{1,3}(,\d{3})+", v)
                    for v in values
                )
                if has_currency:
                    label = "Total"
        ...
        def detect_and_merge_multiindex_headers(data_rows):
            ...
            for i, row in enumerate(data_rows):
                ...
                if row_has_years(row):
                    year_row_idx = i
                    years_list = [
                        c.strip() for c in row if c.strip() and is_year(c.strip())
                    ]
                    data_start = i + 1
                ...
                if row_is_all_caps_categories(row):
                    ...
                    cats = [
                        c.strip()
                        for c in row
                        if c.strip() and is_all_caps_word(c.strip())
                    ]
        ...
        if multiindex_result[0] and len(multiindex_result[0]) >= 1:
            ...
            header_layers, extracted_header_count = extract_periods_from_rows(
                raw_rows_with_colspan, row_has_th_flags
            )
            num_periods = 0
        ...
        def process_equity_statement_table(rows, rows_with_colspan):
            ...
            if not value_positions:
                for j, cell in enumerate(sample_row[:-1]):
                    c = cell.strip()
                    next_c = (
                        sample_row[j + 1].strip() if j + 1 < len(sample_row) else ""
                    )
                    # $ in current cell, number in next
                    if (
                        c == "$"
                        and next_c
                        and re.match(r"^[\(\)]?[\d,]+[\)]?$", next_c)
                    ):
        ...
        def is_data_row_header(row):
            ...
            if re.search(
                r"(years?\s+ended|months?\s+ended|unaudited|as of)", row_text, re.I
            ):
            ...
            period_count = sum(
                1 for c in non_empty if re.match(period_pattern, c, re.I)
            )
        ...
        if header_layers:
            ...
            has_financial_header_terms = any(
                re.search(term, all_header_text, re.I)
                for term in financial_header_terms
            )
        ...
        if header_layers and len(header_layers) == 1:
            ...
            for _h in last_layer[1:]:  # Skip label column
                ...
                if (
                    h
                    and not re.match(r"^(19|20)\d{2}$", h)
                    and not re.search(r"\b(19|20)\d{2}\b", h)
                ):
                    # Not a bare year - check if it's a date+year combo
                    # like "January 26, 2025" or "October 27, 2024"
                    # These are financial period headers, not mixed headers
                    # Non-year value column header (e.g., "Useful Lives", "Salary")
        ...
        if use_semantic_parsing:
            ...
            if header_layers and len(header_layers) >= 2:
                ...
                if len(year_positions) == num_periods and num_periods > 0:
                    ...
                    if year_positions[0] == 0 and _year_pos_shift[0] > 0:
                        year_positions = [
                            p + _year_pos_shift[0] for p in year_positions
                        ]
                ...
                if not header_col_positions and header_layers:
                    ...
                    sub_texts = [
                        col_to_text[cp][0] for cp in sorted(col_to_text.keys())
                    ]
            ...
            if is_financial_periods:
                filtered_rows = [
                    row
                    for row in data[header_row_count:]
                    if not is_data_row_header(row)
                ]
            else:
        ...
        for i, row in enumerate(data):
            ...
            # For multi-index headers (num_header_rows > 1), the separator goes
            # after all header layers so they all appear above the divider.
            if i == 0:
                lines.append("|" + "|".join(["---"] * max_cols) + "|")
        ...
        if tag.name in ["div", "p"]:
            ...
            if not has_bare_text:
                ...
                text_spans = [
                    ...
                    if any(
                        isinstance(c, NavigableString) and c.strip() for c in s.children
                    )
                    and "display:inline-block"
                    not in s.get("style", "").replace(" ", "")
                ]
                if text_spans:
    ...
    def process_element(element, depth=0) -> str:
        ...
        if element.name == "img":
            img_md = convert_image_to_markdown(element)
...
def _classify_table(table_elem) -> str:
    ...
    # If ALL rows are footnote-style, treat as footnote table
    if footnote_rows == len(non_empty_rows) and footnote_rows >= 1:
        return "FOOTNOTE"
...
def _extract_cell_text(
    ...
    # Convert BR tags to a marker that will be preserved
    # This handles cells with multiple items separated by <BR> tags
    ...
    # Convert images to HTML format with size preservation
