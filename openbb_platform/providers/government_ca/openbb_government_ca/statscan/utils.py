...
- ``status``          — ``"ok"`` or ``"degraded"`` (when the build
                        hook couldn't reach StatsCan)
...
def list_indicators(cache: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    callers don't have to know the cache shape. Returns an empty list
    if the section is missing or in degraded mode.
    """
