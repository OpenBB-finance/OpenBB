"""Hatchling build hook: materialize the Canadian government data cache.
...
``openbb_government_ca.utils.generate_cache.main`` against the Bank of
Canada Valet API and the Statistics Canada SDMX/JSON endpoints, then
bundled into the sdist + wheel + editable install.

The hook is a faithful adaptation of the pattern established by
``openbb-oecd`` (PR #7413) for OpenBB V5.

Design notes
------------
- The script is invoked **by path**, not by module name, so the
  isolated PEP 517 build env doesn't import
  ``openbb_government_ca/__init__.py`` (which would pull in
  ``openbb_core`` — not listed as a build-time requirement). The
  generator script is self-contained: stdlib + ``requests``, and
  ``requests`` is in ``[build-system].requires``.
- Hatch runs ``build_sdist`` and ``build_wheel`` in the same Python
  process, so the ``_generated_this_session`` class flag survives
  between target builds and lets the second target reuse the cache
  the first target just produced. Without this guard a release build
  would hit upstream APIs twice (once per target).
"""
...
def _open_tty() -> TextIO | None:
    ...
    completely invisible to the user during a multi-minute upstream
    fetch. Writing to ``/dev/tty`` is the standard escape hatch: it's
    the controlling terminal, untouched by pip's capture pipeline.
    """
...
class GovernmentCaCacheBuildHook(BuildHookInterface):
    ...
    # produced even when ``OPENBB_GOVERNMENT_CA_FORCE_CACHE_REBUILD=1`` is set.
    # Without this guard a release build would fetch from upstream twice
    # (once per target).
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        # ``openbb_government_ca/utils/metadata/_constants.py``'s
        ...
        # first target (sdist) regenerates against live upstream, then the
        # ``_generated_this_session`` flag short-circuits the second
        # target (wheel) into reusing the just-written file. Without this
        # gate a release build hits upstream twice.
        force = (
        ...
        try:
            if _CACHE_PATH.exists() and not force:
                ...
                _say(
                    "government-ca-cache: regenerating from BoC Valet + "
                    "StatsCan (~2-3 min, ~10 API calls)..."
                )
                ...
                if returncode != 0 or not _CACHE_PATH.exists():
                    self.app.abort(
                        ...
                        "(www.bankofcanada.ca/valet) and Statistics "
                        "Canada (www150.statcan.gc.ca) must be "
