"""Federal Reserve Bank of Minneapolis Labor Force Participation Model."""
...
URL = (
    ...
    "/regional-economic-indicators/labor_force_data.csv"
)

_GEOGRAPHIES = ["MN", "MT", "ND", "SD", "WI", "US"]


class FederalReserveMinneapolisLaborForceQueryParams(QueryParams):
    """Minneapolis Fed Labor Force Participation Query Parameters."""
...
class FederalReserveMinneapolisLaborForceFetcher(
    Fetcher[
        FederalReserveMinneapolisLaborForceQueryParams,
        list[FederalReserveMinneapolisLaborForceData],
    ]
):
    """Minneapolis Fed Labor Force Participation Fetcher."""
    ...
    def transform_query(
        ...
        return FederalReserveMinneapolisLaborForceQueryParams(**params)

    @staticmethod
    def extract_data(
        query: FederalReserveMinneapolisLaborForceQueryParams,
        credentials: dict[str, str] | None,
        **kwargs: Any,
    ) -> list[dict]:
        """Download the labor force participation CSV from the Minneapolis Fed."""
        ...
        def _producer() -> str:
            """Fetch the raw labor force CSV text."""
            response = make_request(URL)
            response.raise_for_status()
            return response.text

        text = cached(
            "minneapolis_labor_force",
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        records: list[FederalReserveMinneapolisLaborForceData] = []
        for row in frame.sort_values("date").to_dict(orient="records"):
            ...
            if any(record[column] is not None for column in value_columns):
                records.append(
                    FederalReserveMinneapolisLaborForceData.model_validate(record)
                )
        if not records:
            raise EmptyDataError("The request was returned empty.")
        return records
