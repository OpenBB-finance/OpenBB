"""Federal Reserve Bank of Minneapolis Quits Rate Model."""
...
URL = (
    ...
    "/regional-economic-indicators/quits_rate_data.csv"
)

_GEOGRAPHIES = ["MN", "MT", "ND", "SD", "WI", "US"]


class FederalReserveMinneapolisQuitsRateQueryParams(QueryParams):
    """Minneapolis Fed Quits Rate Query Parameters."""
...
class FederalReserveMinneapolisQuitsRateFetcher(
    Fetcher[
        FederalReserveMinneapolisQuitsRateQueryParams,
        list[FederalReserveMinneapolisQuitsRateData],
    ]
):
    """Minneapolis Fed Quits Rate Fetcher."""
    ...
    def transform_query(
        ...
        return FederalReserveMinneapolisQuitsRateQueryParams(**params)

    @staticmethod
    def extract_data(
        query: FederalReserveMinneapolisQuitsRateQueryParams,
        credentials: dict[str, str] | None,
        **kwargs: Any,
    ) -> list[dict]:
        """Download the quits rate CSV from the Minneapolis Fed."""
        ...
        def _producer() -> str:
            """Fetch the raw quits rate CSV text."""
            response = make_request(URL)
            response.raise_for_status()
            return response.text

        text = cached(
            "minneapolis_quits_rate",
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        records: list[FederalReserveMinneapolisQuitsRateData] = []
        for row in frame.sort_values("date").to_dict(orient="records"):
            ...
            if any(record[column] is not None for column in value_columns):
                records.append(
                    FederalReserveMinneapolisQuitsRateData.model_validate(record)
                )
        if not records:
            raise EmptyDataError("The request was returned empty.")
        return records
