"""BLS Total Factor Productivity news-release chart-data scrapers.

Source: https://www.bls.gov/charts/total-factor-productivity/

Chart specs (slug / label / kind / value columns) are generated from the live
table headers and bundled in ``assets/charts/tfp.json``.
"""
...
_BASE = "https://www.bls.gov/charts/total-factor-productivity"
_SPECS_FILE = Path(__file__).parent.parent / "assets" / "charts" / "tfp.json"
...
def fetch_and_parse(chart_key: str) -> dict[str, Any]:
    ...
    return parse_chart_table(
        ...
        CHART_SPECS,
        period="year",
        error_label="total-factor-productivity",
        table_id_prefix="tfp-chart",
    )
