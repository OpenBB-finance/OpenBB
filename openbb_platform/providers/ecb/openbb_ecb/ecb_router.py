...
@router.command(
    ...
    widget_config={
        "name": "ECB Concepts",
        "description": "The ECB data-portal concepts (topics) and the number of"
        " datasets each one groups.",
        "type": "table",
        "category": "ECB",
        "subCategory": "Catalogue",
        "source": ["ECB"],
        ...
        "data": {
            "table": {
                "columnsDefs": [
                    {"field": "name", "headerName": "Concept"},
                    {"field": "count", "headerName": "Datasets"},
                ]
            }
        },
    },
    examples=[APIEx(description="List ECB concepts (topics).", parameters={})],
)
async def list_concepts(
    metadata: EcbMetadataDependency,
) -> list[dict[str, Any]]:
    """Return the ECB data-portal concepts with their dataset members."""
    return metadata.list_concepts()
...
@router.command(
    ...
    examples=[APIEx(description="ECB concept choices.", parameters={})],
)
async def concept_choices(
    metadata: EcbMetadataDependency,
) -> list[dict[str, str]]:
    """Return ``[{label, value}]`` of ECB concepts for the dropdown."""
    return [{"label": c["name"], "value": c["value"]} for c in metadata.list_concepts()]
...
@router.command(
    ...
    widget_config={"exclude": True},
    include_in_schema=False,
    examples=[APIEx(description="Datasets with data-information.", parameters={})],
)
async def dataflow_info_choices(
    metadata: EcbMetadataDependency,
    concept: Annotated[
        str | None, Query(description="Restrict to a concept's datasets.")
    ] = None,
) -> list[dict[str, str]]:
    """Return ``[{label, value}]`` of dataflows that publish data-information."""
    allowed: set[str] | None = None
    if concept and concept in metadata.portal_concepts:
        allowed = set(metadata.get_concept(concept).get("datasets", []))
    out = [
        {"label": info.get("title") or flow_id, "value": flow_id}
        for flow_id, info in metadata.dataflow_info.items()
        if allowed is None or flow_id in allowed
    ]
...
async def indicator_areas(
    ...
    """Return the reference-area choices for a dataflow's dropdown."""
...
@router.command(
    ...
    widget_config={
        ...
        "description": "The ECB data-portal presentation (release) tables, grouped by"
        " the portal's publication tree — click a row to load it in the ECB"
...
async def list_tables(
    ...
    """List the ECB data-portal presentation tables."""
...
async def list_table_choices(
    ...
    """Return ``[{label, value}]`` of presentation tables."""
    tables = (
...
@router.command(
    ...
    widget_config={
        ...
        "description": "An ECB data-portal presentation (release) table. Every"
        " published series is a row — its title, full definition on hover, unit, and"
        " the most recent periods pivoted into columns as real, unit-scaled numbers.",
        "type": "table",
        "category": "ECB",
        "subCategory": "Presentation Tables",
        "source": ["ECB"],
        ...
        "data": {
            "table": {
                "columnsDefs": [
                    ...
                    {"field": "description", "headerName": "Definition", "hide": True},
                    {
                        "field": "unit",
                        "headerName": "Unit",
                        "pinned": "left",
...
if not FIXEDINCOME_INSTALLED:
    ...
    async def euro_short_term_rate(
        ...
        """Get the euro short-term rate and its detail."""
        return await OBBject.from_query(OBBQuery(**locals()))
...
async def key_interest_rates(
    ...
    """Get the key ECB interest rates."""
    return await OBBject.from_query(OBBQuery(**locals()))
...
@router.command(
    ...
    widget_config={
        "name": "ECB MFI Interest Rates",
        "description": "Euro-area bank (MFI) interest rates on household and NFC"
        " loans and deposits — one column per headline rate (monthly).",
        "type": "table",
        "category": "ECB",
        ...
        "params": [
            ...
            {"paramName": "frequency", "show": False},
        ],
    },
    examples=[APIEx(parameters={"provider": "ecb"})],
)
async def mfi_interest_rates(
    ...
    """MFI interest rates on euro-area loans and deposits."""
    return await OBBject.from_query(OBBQuery(**locals()))


_RELEASE_FEEDS = ("press_releases", "publications", "blog")
...
@router.command(
    ...
    widget_config={"exclude": True},
    include_in_schema=False,
    examples=[APIEx(parameters={"category": "blog"})],
)
async def release_choices(
    category: Annotated[
        str,
        Query(description="ECB feed: press_releases, publications, or blog."),
    ] = "blog",
) -> list[dict[str, str]]:
    """Return ``[{label, value}]`` of recent ECB releases for one feed, newest first."""
    # pylint: disable=import-outside-toplevel
    from openbb_ecb.utils.data_cache import cached_records, make_key
    from openbb_ecb.utils.non_sdmx import fetch_rss_items

    feed = category if category in _RELEASE_FEEDS else "blog"

    async def loader() -> list[dict]:
        return await fetch_rss_items(feed)

    items = await cached_records("releases", make_key("releases", feed=feed), loader)
    items = sorted(items, key=lambda i: i.get("date") or "", reverse=True)
    return [
        {
            "label": f"{(item.get('date') or '')[:10]} · {item['title']}",
            "value": item["url"],
        }
        for item in items
        if item.get("url")
...
async def release_document(
    ...
    """Render one ECB release as a full HTML page for the Workspace HTML widget."""
    # pylint: disable=import-outside-toplevel
    from openbb_ecb.utils.data_cache import cached_records, make_key
    from openbb_ecb.utils.non_sdmx import fetch_release_html, fetch_rss_items

    url = release
    if not url:
        feed = category if category in _RELEASE_FEEDS else "blog"
        items = sorted(
            await fetch_rss_items(feed),
            key=lambda i: i.get("date") or "",
            reverse=True,
        )
        url = next((i["url"] for i in items if i.get("url")), "")

    html = ""
    if url:

        async def loader() -> list[dict]:
            page = await fetch_release_html(url)
            return [{"html": page}] if page else []

        cached = await cached_records(
            "release_html", make_key("release_html", url=url), loader
        )
        html = cached[0]["html"] if cached else ""

    if not html:
        html = (
            '<html><body style="font-family:sans-serif;padding:1.5rem;color:#999">'
            "No release content available.</body></html>"
        )
    return HTMLResponse(content=html)


async def dataflow_information(
    concept: str = "",
    dataflow: str = "",
    theme: str | None = "dark",
) -> HTMLResponse:
    """Render a dataset's rich ECB reference metadata as an HTML page."""
    # pylint: disable=import-outside-toplevel
    from openbb_ecb.utils.metadata import EcbMetadata
    from openbb_ecb.utils.non_sdmx import render_dataflow_info

    metadata = EcbMetadata()
    flow = dataflow
    if not flow and concept and concept in metadata.portal_concepts:
        flow = next(
            (
                d
                for d in metadata.get_concept(concept).get("datasets", [])
                if d in metadata.dataflow_info
            ),
            "",
        )
    info = metadata.get_dataflow_info(flow) if flow else None
    if not info:
        return HTMLResponse(
            content='<html><body style="font-family:sans-serif;padding:1.5rem;'
            'color:#999">No data information available.</body></html>'
        )
    html = render_dataflow_info(flow, info, metadata.concepts_for_dataflow(flow))
    return HTMLResponse(content=html)


router._api_router.add_api_route(
    path="/dataflow_information",
    endpoint=dataflow_information,
    methods=["GET"],
    response_class=HTMLResponse,
    openapi_extra={
        "widget_config": {
            "name": "ECB Dataset Information",
            "description": "The ECB's full reference metadata for a dataset — scope,"
            " methodology, statistical processing, legal basis, dissemination and the"
            " downloadable series catalogue.",
            "category": "ECB",
            "subCategory": "Catalogue",
            "type": "html",
            "widgetId": "ecb_dataflow_information",
            "params": [
                {
                    "paramName": "concept",
                    "label": "Concept",
                    "show": True,
                    "type": "endpoint",
                    "optionsEndpoint": f"{api_prefix}/ecb/concept_choices",
                ...
                },
                {
                    ...
                    "show": True,
                    "type": "endpoint",
                    "optionsEndpoint": f"{api_prefix}/ecb/dataflow_info_choices",
                    "optionsParams": {"concept": "$concept"},
                    "style": {"popupWidth": 600},
                },
...
router._api_router.add_api_route(
    path="/release_document",
    endpoint=release_document,
    methods=["GET"],
    response_class=HTMLResponse,
    openapi_extra={
        "widget_config": {
            "name": "ECB Release Viewer",
            "description": "Read any ECB press release, publication, or blog post in"
            " full — charts, tables and images included.",
            "category": "ECB",
            "subCategory": "Releases",
            "type": "html",
            "widgetId": "ecb_release_document",
            "params": [
                {
                    "paramName": "category",
                    "label": "Feed",
                ...
                },
                {
                    ...
                    "show": True,
                    "type": "endpoint",
                    "optionsEndpoint": f"{api_prefix}/ecb/release_choices",
                    "optionsParams": {"category": "$category"},
                    "style": {"popupWidth": 800},
                },
                {"paramName": "theme", "show": False},
            ],
            "gridData": {"w": 40, "h": 24},
...
async def eligible_assets(
    ...
    """Get the Eurosystem list of eligible marketable assets."""
    return await OBBject.from_query(OBBQuery(**locals()))
...
async def get_ecb_apps_json() -> list[dict[str, Any]]:
    """Serve the bundled OpenBB Workspace app definitions."""
    import json
