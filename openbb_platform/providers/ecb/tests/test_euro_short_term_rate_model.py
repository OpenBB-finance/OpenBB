"""Unit tests for the ECB euro short-term rate model."""
...
def test_transform_pivots_and_scales():
    """Data-type series pivot to one record per date with percent scaling."""
    data = [
        _record("WT", 2.182),
        _record("R25", 2.16),
        _record("R75", 2.2),
        ...
        _record("NB", 44),
        _record("VL", 43),
        _record("ZZ", 1),  # unknown data type -> ignored
        _record(None, 5),  # no data type -> ignored
        {"DATA_TYPE_EST": "WT", "OBS_VALUE": None, "date": "2025-01-03"},  # no rate row
    ]
    out = Fetcher.transform_data(Fetcher.transform_query({}), data)
    assert len(out) == 1  # the 2025-01-03 row has no rate and is dropped
    row = out[0]
    assert round(row.rate, 5) == 0.02182  # /100
...
def test_aextract(monkeypatch):
    """Extract returns the raw €STR records."""
