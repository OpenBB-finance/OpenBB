...
from openbb_government_us.congress import congress_router as router
...
def test_bill_text_urls_empty_workspace():
    """A missing bill_id in workspace mode returns a placeholder."""
    result = asyncio.run(router.bill_text_urls(bill_id=None, is_workspace=True))
    assert result[0]["value"] is None


def test_bill_text_urls_empty_raises():
    """A missing bill_id outside workspace raises an HTTPException."""
    with pytest.raises(HTTPException):
        asyncio.run(router.bill_text_urls(bill_id=None))
...
def test_bill_text_urls_delegates(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.helpers.get_bill_text_choices", _fake
    )
...
def test_amendment_text_urls_empty_workspace():
    """A missing amendment_id in workspace mode returns a placeholder."""
    result = asyncio.run(
        router.amendment_text_urls(amendment_id=None, is_workspace=True)
    )
    assert result[0]["value"] is None


def test_amendment_text_urls_empty_raises():
    """A missing amendment_id outside workspace raises an HTTPException."""
    with pytest.raises(HTTPException):
        asyncio.run(router.amendment_text_urls(amendment_id=None))
...
def test_amendment_text_urls_delegates(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.helpers.get_amendment_text_choices", _fake
    )
...
def test_committee_document_urls_delegates(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.committees.get_committee_doc_choices",
        _fake,
    )
    result = asyncio.run(
        router.committee_document_urls(
...
def test_search_document_urls_empty_workspace():
    """A missing package id in workspace mode returns a select-a-row placeholder."""
    result = asyncio.run(
        router.search_document_urls(package_id=None, is_workspace=True)
    )
    assert result == [{"label": "Select a row to view the document.", "value": None}]


def test_search_document_urls_empty_raises():
    """A missing package id outside workspace raises an HTTPException."""
    with pytest.raises(HTTPException):
        asyncio.run(router.search_document_urls(package_id=None))
...
def _patch_fetch_cmr(monkeypatch, records):
    ...
    monkeypatch.setattr("openbb_government_us.congress.utils.bulk.fetch_cmr", _fake)
...
def test_mandated_report_urls_empty_workspace(monkeypatch):
    ...
    result = asyncio.run(
        router.mandated_report_urls(package_id=None, is_workspace=True)
    )
    assert result == [{"label": "No documents available.", "value": None}]
...
def test_mandated_report_urls_empty_non_workspace(monkeypatch):
    ...
    _patch_fetch_cmr(monkeypatch, [])
    assert asyncio.run(router.mandated_report_urls(package_id=None)) == []
...
def test_mandated_report_urls_fallback(monkeypatch):
    ...
    ws = asyncio.run(router.mandated_report_urls(package_id=None, is_workspace=True))
    assert ws[0]["label"] == "Report One - CMR-A98-1.pdf"
    assert ws[0]["value"] == "https://x/r.pdf"
    raw = asyncio.run(router.mandated_report_urls(package_id=None, congress=119))
...
def _patch_load_plaw(monkeypatch, public=(), private=()):
    ...
    monkeypatch.setattr("openbb_government_us.congress.utils.bulk.load_plaw", _fake)


def test_law_text_urls_empty_workspace_placeholder(monkeypatch):
    """A missing law_id with no recent laws returns the workspace placeholder."""
    _patch_load_plaw(monkeypatch)
    result = asyncio.run(router.law_text_urls(law_id=None, is_workspace=True))
    assert result == [{"label": "No documents available.", "value": None}]


def test_law_text_urls_empty_non_workspace(monkeypatch):
    """A missing law_id with no recent laws returns an empty non-workspace list."""
    _patch_load_plaw(monkeypatch)
    assert asyncio.run(router.law_text_urls(law_id=None)) == []
...
def _patch_load_calendars(monkeypatch, records):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.load_calendars", _fake
    )
...
def test_calendar_document_urls_empty_workspace_placeholder(monkeypatch):
    ...
    result = asyncio.run(
        router.calendar_document_urls(package_id=None, is_workspace=True)
    )
    assert result == [{"label": "No documents available.", "value": None}]
...
def test_calendar_document_urls_fallback(monkeypatch):
    ...
    out = asyncio.run(router.calendar_document_urls(package_id=None, congress=119))
...
def test_preload_bills(monkeypatch):
    ...
    from openbb_government_us.congress.utils.constants import BillTypes
...
def _patch_member_warmup(monkeypatch, *, members):
    ...
    base = "openbb_government_us.congress.utils.bulk."
...
def test_preload_members_handles_failed_reference_load(monkeypatch):
    ...
    monkeypatch.setattr("openbb_government_us.congress.utils.bulk.load_members", _boom)
...
def test_warmup_orders_preloads_and_schedules_loops(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.seed_billstatus_markers", _seed
    )
...
def test_refresh_loop_logs_error_then_cancels(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.refresh_billstatus", _refresh
    )
...
def test_passage_refresh_loop_logs_error_then_cancels(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.refresh_passage", _refresh
    )
    monkeypatch.setattr(asyncio, "sleep", _ticking_sleep())
    with pytest.raises(asyncio.CancelledError):
...
def test_member_choices(monkeypatch):
    ...
    monkeypatch.setattr("openbb_government_us.congress.utils.bulk.load_members", _load)
...
def test_member_info_html_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.load_member_record", _record
    )
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.member_committees", _committees
    )
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.load_social_media", _social
    )
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.member_passage_record", _passage
    )
    monkeypatch.setattr(
        "openbb_government_us.congress.utils.bulk.member_photo_url", _photo
    )
