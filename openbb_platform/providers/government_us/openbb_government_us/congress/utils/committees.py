...
async def get_committee_overview(system_code: str, chamber: str) -> dict:
    ...
    from openbb_government_us.congress.utils.bulk import load_committee_structure
...
async def get_committee_doc_choices(
    ...
    from openbb_government_us.congress.utils.bulk import fetch_package_mods, parse_mods

    docs = await fetch_committee_documents(system_code, congress, doc_type, limit=limit)

    if not docs:
        if is_workspace is True:
            return [{"label": "No documents found for this committee.", "value": None}]
        return []
