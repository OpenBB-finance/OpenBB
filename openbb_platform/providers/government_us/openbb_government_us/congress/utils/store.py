...
def _bill_list_item(row) -> dict:
    ...
    return {
        ...
        "type": row[5],
        "title": row[6],
        "latestAction": {"actionDate": row[8] or None, "text": row[9] or None},
        "updateDateIncludingText": row[10] or None,
    }
        ...
        return [
            {
                ...
                "role": row[2],
                "title": row[3],
                "introduced_date": row[4] or None,
                "latest_action_date": row[5] or None,
                "latest_action": row[6] or None,
            }
            for row in rows
        ]
    finally:
