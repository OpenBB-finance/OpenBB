...
class CongressAmendmentsData(NullTokenMixin, Data):
    ...
    submitted_date: dateType | None = Field(
        ...
        json_schema_extra={"x-widget_config": {"headerName": "Submitted"}},
    )
    ...
    amendment_type: str = Field(
        ...
        json_schema_extra={"x-widget_config": {"headerName": "Type"}},
    )
    number: str = Field(
        description="The amendment number.",
        json_schema_extra={
            "x-widget_config": {"formatterFn": "none", "headerName": "Amendment No."},
        },
    )
    ...
    amended_bill_title: str | None = Field(
        ...
        json_schema_extra={"x-widget_config": {"headerName": "Bill Title"}},
    )
...
class CongressAmendmentsFetcher(
    ...
    def transform_data(
        ...
        def sort_key(record: dict) -> str:
            """Return the submitted date, falling back to the action or update date."""
            return (
                record.get("submittedDate")
                or (record.get("latestAction") or {}).get("actionDate")
