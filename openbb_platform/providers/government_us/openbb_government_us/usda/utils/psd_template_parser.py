...
from openbb_government_us.usda.utils.psd_codes import REGION_DISPLAY
...
def set_region_country(
    ...
    -------
    tuple[str | None, str]
        (normalized_region_name, "--") if entity_name is a known region,
        ("European Union", "European Union") if entity_name is "European Union",
        (parent_region, entity_name) otherwise.
    """
...
def parse_template_1(
    ...
    """Parse templates 1 and 20, area, yield, and production tables."""
    import re
...
def parse_template_3(
    ...
    """Parse templates 3 and 17, supply and distribution by country/year."""
    report_title = lines[0].strip()
    commodity = extract_commodity_from_title(report_title, commodity_group) or "Unknown"
...
def parse_template_5(
    ...
    """Parse template 5, world trade/production tables in commodity view."""
    report_title = lines[0].strip()
...
def parse_template_7(
    ...
    """Parse template 7, production/consumption summary in country view."""
    report_title = lines[0].strip()
...
def parse_template_8(
    ...
    """Parse template 8, summary tables like Coffee and Cotton World Supply."""
    report_title = lines[0].strip()
...
def parse_template_13(
    ...
    """Parse template 13, multi-commodity supply/demand tables."""
    report_title = lines[0].strip()
...
def parse_template_2(
    ...
    """Parse template 2, World Crop Production Summary."""
    report_title = lines[0].strip()
...
def parse_template_9(
    ...
    """Parse template 9, Copra, Palm Kernel, and Palm Oil Production."""
    import re
...
def parse_template_11(
    ...
    """Parse template 11, All Grain Summary Comparison."""
    report_title = lines[0].strip()
...
def parse_template_20(
    ...
    """Parse template 20, Total Oilseed Area, Yield, and Production."""
    import re
    ...
    data = []
    current_region = None
    current_commodity = "Total Oilseeds"
...
def parse_report(
    ...
    ----------
    template_id : int
        The template ID for the report.
    lines : list
        CSV lines from the report.
    html_text : str | None
        HTML text from the report (for unit extraction).
    commodity_group : str | None
        The commodity group code (e.g., 'cot', 'crn', 'wht').
    """
