...
def run_bootstrap(
    config: dict[str, Any], state: JobState, run_id: str, run_dir
) -> None:
    """Execute bootstrap steps: warm/cache/features/train/promote."""
    defaults = config.get("defaults", {}) if isinstance(config, dict) else {}
    weekly_cfg = config.get("weekly", {}) if isinstance(config, dict) else {}
    runtime_cfg: dict[str, Any] = {}
    if isinstance(defaults, dict):
        runtime_cfg.update(defaults)
    if isinstance(weekly_cfg, dict):
    ...
    if not bool(runtime_cfg.get("pretrain_bootstrap_enabled", True)):
        ...
        write_json(run_dir, "time_profile.json", {})
        return
    ...
    for step_name, func in steps:
        ...
        if (
            ...
            state.set("bootstrap.candidate_run_id", str(result["run_id"]))
        if (
            ...
            state.set("bootstrap.latest_run_id", str(result["run_id"]))
