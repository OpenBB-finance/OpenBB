...
def _solve_mv_with_cvxpy(
    *,
    ...
    objective = cp.Maximize(
        mu @ w
        - float(risk_aversion) * cp.quad_form(w, cov)
        - float(linear_rate) * cp.norm1(delta)
        - cp.sum(cp.multiply(impact_scale, cp.square(delta)))
        - turnover_term
    )
    ...
    if beta_neutral and beta_vector is not None and len(beta_vector) == n_assets:
        constraints.append(cp.abs(beta_vector @ w - float(target_beta)) <= float(beta_tolerance))
    ...
    if not solved or w.value is None:
        raise ValueError("mv_solver_failed")
...
def optimize_weights_v2(
    *,
    ...
    mv_optimizer_engine: str = "legacy_slsqp",
    ...
    country_cap = _safe_float(
        exp_cfg.get("country_max_weight"),
        _safe_float(p_cfg.get("country_cap"), 0.35),
    )
    ...
    requested_mv_engine = str(mv_optimizer_engine or "legacy_slsqp").strip().lower()
    mode_used = "mv"
    ...
    if requested_mode == "sharpe":
        requested_mode = "mv"

    if requested_mode == "risk_parity":
        asset_vol = np.sqrt(np.clip(np.diag(cov), EPS, None))
        inv_vol = 1.0 / np.clip(asset_vol, np.sqrt(EPS), None)
        inv_vol = np.nan_to_num(inv_vol, nan=0.0, posinf=0.0, neginf=0.0)
        weights = _projected_fallback(
            inv_vol,
            np.clip(ub, 0.0, None),
            sectors,
            countries,
            sector_cap=max(0.0, sector_cap),
            country_cap=max(0.0, country_cap),
        )
        mode_used = "risk_parity"
    ...
    if weights.size == 0 and requested_mode != "cvar" and requested_mv_engine in {"cvxpy", "auto"}:
        impact_scale = (impact_k / 1e4) / np.clip(adv_capacity, EPS, None)
        try:
            weights = _solve_mv_with_cvxpy(
                mu=mu,
                cov=cov,
                lb=lb,
                ub=ub,
            ...
            if bool(optimizer_strict):
                raise ValueError(f"mv_cvxpy_failed:{exc}") from exc
    ...
    if mode_used == "risk_parity":
        binding.append("optimizer_risk_parity")
    if used_cvar_fallback:
        binding.append("optimizer_fallback_mv")
    if used_mv_cvxpy:
        binding.append("optimizer_mv_cvxpy")
    if used_mv_fallback:
        binding.append("optimizer_mv_fallback_slsqp")
    if target_vol > 0:
