...
def quantile_anamoly_detection(
    ...
    anom_results = QuantileAnomalyModel(
        ...
        anomaly_score=forecast_helpers.timeseries_to_basemodel(anom_score),
        binary_anomaly_prediction=forecast_helpers.timeseries_to_basemodel(binary_anom),
    )
...
def autoarima(
    ...
    precision = forecast_helpers.mean_absolute_percentage_error(y_true, y_hat)
    ...
    _, forecast = forecast_helpers.convert_to_timeseries(
        forecast.rename(columns={"AutoARIMA": target_column}),
        target_column,
        is_scaler=use_scalers,
        time_col="ds",
    )
    _, historical_fcast = forecast_helpers.convert_to_timeseries(
    ...
    results = StatisticalForecastModel(
        ...
        forecast=forecast_helpers.timeseries_to_basemodel(forecast),
        precision=precision,
        forecast_model=fcst,
    )
...
def autoets(
    ...
    _, historical_fcast_ets = forecast_helpers.convert_to_timeseries(
