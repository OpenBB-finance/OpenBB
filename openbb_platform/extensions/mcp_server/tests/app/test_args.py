...
class TestExpandSpecHeaders:
    """``$VAR`` expansion in spec headers."""

    def test_expand_spec_headers_substitutes(self, monkeypatch):
    ...
    def test_expand_spec_headers_skips_unresolved(self, monkeypatch, caplog):
    ...
    def test_expand_spec_headers_falsy_returns_empty(self):
        """None / empty dict → empty dict."""
        assert _expand_spec_headers(None) == {}
        assert _expand_spec_headers({}) == {}

    def test_expand_spec_headers_skips_non_string_keys(self):
...
class TestIsModuleColonNotation:
    """Telling ``module:attr`` from a Windows drive letter."""

    def test_is_module_colon_notation_detects_real_colon_notation(self):
    ...
    def test_is_module_colon_notation_rejects_bare_paths_and_drives(self):
    ...
    def test_is_module_colon_notation_drive_letter_with_attr_suffix(self):
        r"""``C:\path\app.py:create_app`` has two colons; treat as module:attr."""
        assert _is_module_colon_notation(r"C:\path\app.py:create_app") is True


class TestCollectMultiSpecEntries:
    """Selecting multi-spec subtables."""

    def test_collect_multi_spec_entries_filters_non_spec_subtables(self):
...
class TestLaunchScriptDescription:
    """The ``--help`` text."""

    def test_launch_script_description_documents_key_flags(self):
...
class TestParseArgs:
    """Parsing the launcher's arguments and TOML defaults."""

    def test_parse_args_returns_expected_dict_shape(self):
    ...
    def test_parse_args_help_short_circuits(self, capsys):
    ...
    def test_parse_args_use_colors_pair(self):
    ...
    def test_parse_args_boolean_string_promotion(self):
    ...
    def test_parse_args_json_value_decoded(self):
        """JSON-shaped values get json.loads'd."""
        with patch.object(
            sys, "argv", ["openbb-mcp", "--default_categories", '["a","b"]']
        ):
            out = parse_args()
            assert out["mcp_overrides"]["default_categories"] == ["a", "b"]

    def test_parse_args_invalid_json_falls_back_to_string(self):
    ...
    def test_parse_args_bare_flag_is_true(self):
    ...
    def test_parse_args_mcp_overrides_routed(self):
    ...
    def test_parse_args_uvicorn_passthrough_for_unknowns(self):
    ...
    def test_parse_args_spec_loads_app(self, tmp_path):
    ...
    def test_parse_args_spec_relative_path_resolved(self, tmp_path, monkeypatch):
    ...
    def test_parse_args_app_imports_user_supplied(self, tmp_path):
    ...
    def test_parse_args_spec_and_app_mutually_exclusive(self, tmp_path):
    ...
    def test_parse_args_factory_without_name_raises(self, tmp_path):
    ...
    def test_parse_args_factory_no_name_with_windows_drive_path(self):
    ...
    def test_parse_args_loads_mcp_table_from_explicit_config(self, tmp_path):
    ...
    def test_parse_args_config_tables_stay_out_of_uvicorn(self, tmp_path):
        """``[mcp.spec]``, ``[mcp.auth]``, and ``[mcp.middleware]`` never reach uvicorn."""
        spec = tmp_path / "t.spec"
        spec.write_text(json.dumps(SAMPLE_SPEC))
        cfg = tmp_path / "openbb.toml"
        cfg.write_text(
            f"""
...
path = "{spec.as_posix()}"
[mcp.auth]
hooks = ["my_pkg.auth:check"]
[mcp.middleware]
hooks = ["my_pkg.middleware:log"]
"""
        )
        with patch.object(sys, "argv", ["openbb-mcp", "--config-file", str(cfg)]):
            out = parse_args()
        assert out["uvicorn_overrides"] == {"port": 7000}
        assert out["app"].state.openbb_spec_source == "t.spec"

    def test_parse_args_loads_spec_from_mcp_spec_table(self, tmp_path):
    ...
    def test_parse_args_loads_spec_headers_from_table(self, tmp_path, monkeypatch):
...
'''
    ...
    def test_parse_args_passes_deploy_config_hash_pin_to_load_spec(self, tmp_path):
...
"""
    ...
    def test_parse_args_deploy_config_hash_pin_matches(self, tmp_path):
...
"""
    ...
    def test_parse_args_loads_multi_spec_from_named_subtables(self, tmp_path):
...
"""
    ...
    def test_parse_args_multi_spec_per_spec_content_sha256_pin(self, tmp_path):
...
"""
    ...
    def test_parse_args_multi_spec_with_per_spec_hooks(self, tmp_path, monkeypatch):
...
"""
    ...
    def test_parse_args_supports_top_level_spec_table(self, tmp_path):
    ...
    def test_parse_args_app_with_colon_strips_name_from_path(self, tmp_path):
    ...
    def test_parse_args_module_colon_notation_extracts_name(self, tmp_path: Path):
