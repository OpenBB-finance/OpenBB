...
class TestResolveEntrypoint:
    """Resolving ``module:attr`` hook references."""

    def test_resolve_entrypoint_returns_attribute(self, fake_hook_module):
    ...
    def test_resolve_entrypoint_walks_dotted_attr(self, fake_hook_module):
        """``module:Class.method`` resolves the dotted attribute chain."""
        name, module = fake_hook_module

        class Thing:
            async def m(self, request, call_next):
                return await call_next(request)
    ...
    def test_resolve_entrypoint_requires_colon(self):
        """Missing ``:`` separator raises ValueError."""
        with pytest.raises(ValueError, match="module:attr"):
            _resolve_entrypoint("no_colon")

    def test_resolve_entrypoint_module_not_importable(self):
    ...
    def test_resolve_entrypoint_attribute_missing(self, fake_hook_module):
...
class TestValidateMiddlewareCallable:
    """Validating hook callables."""

    def test_validate_middleware_callable_accepts_async(self):
    ...
    def test_validate_middleware_callable_rejects_non_callable(self):
    ...
    def test_validate_middleware_callable_rejects_sync(self):
    ...
    def test_validate_middleware_callable_rejects_low_arity(self):
...
class TestHookToMiddleware:
    """Wrapping hooks as Starlette middleware."""

    def test_hook_to_middleware_wraps_in_base_http_middleware(self):
...
class TestBuildHookMiddleware:
    """Ordering auth and middleware hooks."""

    def test_build_hook_middleware_empty_returns_empty_list(self):
        """No hooks returns an empty list."""
        assert build_hook_middleware(None, None) == []
        assert build_hook_middleware([], []) == []

    def test_build_hook_middleware_auth_runs_before_middleware(self, fake_hook_module):
        """Auth hooks register before middleware hooks."""
        name, module = fake_hook_module

        async def auth_a(request, call_next):
            return await call_next(request)
    ...
    def test_build_hook_middleware_within_list_order_preserved(self, fake_hook_module):
    ...
    def test_build_hook_middleware_rejects_non_list_table(self):
        """Non-list ``hooks`` value raises TypeError."""
        with pytest.raises(TypeError, match="must be a list"):
            build_hook_middleware(auth_hooks="not a list", middleware_hooks=None)

    def test_build_hook_middleware_rejects_non_string_entry(self):
