...
def _is_module_colon_notation(app_path: str) -> bool:
    """Tell a ``module:attr`` colon from a Windows drive-letter colon."""
    if ":" not in app_path:
        return False
...
def _collect_multi_spec_entries(spec_section: dict) -> dict[str, dict]:
    """Return the ``[mcp.spec.NAME]`` subtables that carry a ``path`` string."""
    out: dict[str, dict] = {}
...
def _expand_spec_headers(
    ...
    """Expand ``$VAR`` / ``${VAR}`` refs in ``[mcp.spec.headers]``, skipping unresolved entries."""
...
Launcher specific arguments:
    ...
    --port                          Port for HTTP / SSE transports. Default is 8001.
    ...
    --tool-discovery                Hide the tools behind available_categories, available_tools, search_tools, and call_tool.
...
MCP-specific tables:
    ...
    [mcp.auth]
    # ``async def hook(request, call_next)`` entry points, run on the
    # HTTP and SSE transports (not stdio), before the middleware hooks.
...
Every other ``--key value`` is routed by name: an MCPSettings field
(``--enable-cli-tools false``, ``--server-auth '["user", "pass"]'``) sets
that setting, ``--httpx-<option>`` (``--httpx-verify false``) sets an
option of the httpx client that calls the API, and anything else is passed
to uvicorn.
"""
...
def parse_args() -> dict:
    ...
    -------
    dict
        ``app`` (the FastAPI app from ``--app`` or a spec, else None), ``transport``,
        ``mcp_overrides``, and ``uvicorn_overrides``.
    """
    ...
    cli_spec = _kwargs.pop("spec", None)
    for table in ("auth", "middleware"):
        _kwargs.pop(table, None)

    spec_section: dict = (
        (layered.get("mcp") or {}).get("spec") or layered.get("spec") or {}
    )
    multi_specs = _collect_multi_spec_entries(spec_section)
    single_spec_path = spec_section.get("path") or (
        cli_spec if isinstance(cli_spec, str) else None
    )
