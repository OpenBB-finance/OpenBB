"""Integration-test fixtures for openbb-econometrics."""
...
def _timeseries_records() -> list[dict]:
    ...
    t = np.arange(_N_ROWS)
    base = 100 + 0.5 * t + rng.normal(0, 2, _N_ROWS).cumsum() * 0.3
    close = base + rng.normal(0, 1, _N_ROWS)
...
def timeseries_data() -> list[dict]:
    """Deterministic OHLCV records for the time-series command tests."""
    return _timeseries_records()


@pytest.fixture(scope="session")
def panel_data() -> list[dict]:
    """Deterministic entity/time panel records as JSON-safe dicts."""
    np.random.seed(_SEED)
    return [
        row.model_dump(mode="json") for row in df_to_basemodel(mock_multi_index_data())
    ]
