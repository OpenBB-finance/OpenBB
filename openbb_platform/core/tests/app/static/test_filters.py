...
#
# The fix: add choices validation to `filter_inputs`, which runs on the raw user kwargs
# dict BEFORE the ExtraParams dataclass is built, so the explicitly-passed "monthly"
...
# after OECD's __json_schema_extra__ gained {"frequency": {"choices": [...]}}.
...
# Info dict WITHOUT choices — mirrors the pre-fix `economy.py` state where
# __json_schema_extra__ had no explicit choices for BOP frequency.
...
def test_filter_inputs_pre_fix_monthly_passed_without_choices():
    ...
    This proves filter_inputs alone was the only place to catch it: once 'monthly'
    exits filter_inputs, filter_extra_params sees v == merged_default ("monthly") and
    silently drops the parameter, so the OECD model never validates it.
    """
    kwargs = {
        "provider_choices": {"provider": "oecd"},
        "standard_params": {},
...
def test_filter_inputs_post_fix_monthly_rejected_with_choices():
    ...
    Together with the test above this pair proves:
