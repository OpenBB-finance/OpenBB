"""Coverage API router helper functions."""
from inspect import _empty, signature
from typing import Any, Dict, Tuple, Type

from openbb_core.app.provider_interface import ProviderInterface
from pydantic import BaseModel, Field, create_model

provider_interface = ProviderInterface()


def get_route_callable(route: str):
    """Get the callable for a route."""
    split_route = route.replace(".", "/").split("/")[1:]
    from openbb import obb  # pylint: disable=import-outside-toplevel

    return_callable = obb

    for route_path in split_route:
        return_callable = getattr(return_callable, route_path)

    return return_callable


def signature_to_fields(route: str) -> Dict[str, Tuple[Any, Field]]:
    """Convert a command signature to pydantic fields."""
    return_callable = get_route_callable(route)
    sig = signature(return_callable)

    fields = {}
    for name, param in sig.parameters.items():
        if name not in ["kwargs", "args"]:
            type_annotation = (
                param.annotation if param.annotation is not _empty else Any
            )
            description = (
                param.annotation.__metadata__[0].description
                if hasattr(param.annotation, "__metadata__")
                else None
            )
            fields[name] = (
                type_annotation,
                Field(..., title="openbb", description=description),
            )

    return fields


def dataclass_to_fields(model_name: str) -> Dict[str, Tuple[Any, Field]]:
    """Convert a dataclass to pydantic fields."""
    dataclass = provider_interface.params[model_name]["extra"]
    fields = {}
    for name, field in dataclass.__dataclass_fields__.items():
        type_annotation = field.default.annotation if field.default is not None else Any
        description = field.default.description if field.default is not None else None
        title = field.default.title if field.default is not None else None
        fields[name] = (
            type_annotation,
            Field(..., title=title, description=description),
        )

    return fields


def create_combined_model(
    model_name: str, *field_sets: Dict[str, Tuple[Any, Field]]
) -> Type[BaseModel]:
    """Create a combined pydantic model."""
    combined_fields = {}
    for fields in field_sets:
        combined_fields.update(fields)

    model = create_model(model_name, **combined_fields)

    # Clean up the metadata
    for field in model.model_fields.values():
        field.metadata = None

    return model


def get_route_schema_map(
    command_model_map: Dict[str, str]
) -> Dict[str, Dict[str, Any]]:
    """Get the route schema map."""
    route_schema_map = {}
    for route, model in command_model_map.items():
        input_model = create_combined_model(
            route, signature_to_fields(route), dataclass_to_fields(model)
        )
        output_model = provider_interface.return_schema[model]
        return_callable = get_route_callable(route)

        route_schema_map[route] = {
            "input": input_model,
            "output": output_model,
            "callable": return_callable,
        }

    return route_schema_map
