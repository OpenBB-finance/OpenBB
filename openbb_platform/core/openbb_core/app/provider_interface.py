...
class ProviderInterface(metaclass=SingletonMeta):
    ...
    def _generate_model_providers_dc(self, map_: MapType) -> dict[str, ProviderChoices]:
        ...
        for model_name, providers in map_.items():
            ...
            choices = self._with_auto_choice(choices)
    ...
    @staticmethod
    def _with_auto_choice(choices: list[str]) -> list[str]:
        """Ensure provider choices always include auto for strategy routing."""
        if "auto" in choices:
            return choices
        return ["auto", *choices]

    def _get_provider_choices(self, available_providers: list[str]) -> type:
        choices = self._with_auto_choice(available_providers)
        return make_dataclass(
            cls_name="ProviderChoices",
            fields=[("provider", Literal[tuple(choices)], "auto")],  # type: ignore
            bases=(ProviderChoices,),
        )
