...
def __getattr__(name: str) -> Any:
    """Resolve dynamic OBBject_* return annotation aliases on demand.

    Autogenerated package modules may import symbols like ``OBBject_Foo`` from
    this module. These names are derived from registry models at runtime, so we
    lazily expose them here for import-time compatibility.
    """
    if name.startswith("OBBject_"):
        model_name = name.replace("OBBject_", "", 1)
        annotations = ProviderInterface().return_annotations
        if model_name in annotations:
            value = annotations[model_name]
            globals()[name] = value
            return value
