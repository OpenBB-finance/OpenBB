...
pred_controller = pytest.importorskip(
    modname="gamestonk_terminal.etf.prediction_techniques.pred_controller",
...
def test_menu_with_queue(expected, mocker, queue):
    mocker.patch(
        target=(
            "gamestonk_terminal.etf.prediction_techniques.pred_controller."
...
def test_menu_without_queue_completion(mocker):
    ...
    mocker.patch.object(
        target=pred_controller.obbff,
        attribute="USE_PROMPT_TOOLKIT",
        new=True,
    )
    mocker.patch(
        target="gamestonk_terminal.etf.prediction_techniques.pred_controller.session",
    )
    mocker.patch(
        target="gamestonk_terminal.etf.prediction_techniques.pred_controller.session.prompt",
        return_value="quit",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("q", ["quit"]),
        ("h", []),
        ("r", ["quit", "quit", "reset", "etf", "load ARKW", "pred"]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "call_reset",
            [],
            ["quit", "quit", "reset", "etf", "load ARKW", "pred"],
        ),
        (
            "call_reset",
            ["help"],
            ["quit", "quit", "reset", "etf", "load ARKW", "pred", "help"],
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            dict(
                ticker="MOCK_TICKER",
                values=PRED_CONTROLLER.data["AdjClose"],
                n_predict=1,
                trend="A",
                seasonal="M",
                seasonal_periods=2,
        ...
        (
            ...
            dict(
                ticker="MOCK_TICKER",
                data=PRED_CONTROLLER.data["AdjClose"],
                n_neighbors=4,
                n_input_days=1,
                n_predict_days=2,
                test_size=0.1,
...
def test_call_func(tested_func, mocked_func, other_args, called_with, mocker):
    mock = mocker.Mock()
    mocker.patch(
        "gamestonk_terminal.etf.prediction_techniques.pred_controller." + mocked_func,
        new=mock,
    )
