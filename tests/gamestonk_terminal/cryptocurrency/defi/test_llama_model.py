...
@pytest.mark.parametrize(
    "func, kwargs",
    [
        ("get_defi_protocols", dict()),
        ("get_defi_protocol", dict(protocol="anchor")),
        ("get_defi_tvl", dict()),
    ],
)
def test_call_func_status_400(func, kwargs, mocker):
    # MOCK GET
    attrs = {
        "status_code": 400,
    }
    ...
    with pytest.raises(Exception) as _:
        getattr(llama_model, func)(**kwargs)
...
def test_call_func_value_error(func, kwargs, mocker):
    # MOCK GET
    attrs = {
        "status_code": 200,
