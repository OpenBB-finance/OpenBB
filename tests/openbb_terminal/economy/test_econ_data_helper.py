...
from openbb_terminal.economy import econ_data_helper
...
def test_draw_graph(mocker):
    ...
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.mpf.plot",
        new=mock_plot,
    )
...
def test_draw_graph_cache_with_file(mocker):
    # MOCK IS_FILE
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.os.path.isfile",
        return_value=True,
    )

    # MOCK READ_PICKLE
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.pd.read_pickle",
...
def test_draw_graph_cache_without_file(mocker):
    ...
    # MOCK LOAD_TICKER
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.stocks_helper.load_ticker",
        return_value=STOCK_DF,
    )

    # MOCK FIND_TRENDLINE
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.stocks_helper.find_trendline",
        return_value=STOCK_DF,
    )

    # MOCK TO_PICKLE
    mocker.patch(
        target="openbb_terminal.economy.econ_data_helper.pd.DataFrame.to_pickle",
        return_value=STOCK_DF,
    )
