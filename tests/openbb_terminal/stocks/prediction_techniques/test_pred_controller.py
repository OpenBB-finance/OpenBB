...
try:
    from openbb_terminal.stocks.prediction_techniques import pred_controller
except ImportError:
...
def test_menu_with_queue(expected, mocker, queue):
    mocker.patch(
        target=(
            "openbb_terminal.stocks.prediction_techniques.pred_controller."
...
def test_menu_without_queue_completion(mocker):
    ...
    mocker.patch(
        target="openbb_terminal.stocks.prediction_techniques.pred_controller.session",
    )
    mocker.patch(
        target="openbb_terminal.stocks.prediction_techniques.pred_controller.session.prompt",
        return_value="quit",
    )
...
def test_call_func(tested_func, mocked_func, other_args, called_with, mocker):
    mock = mocker.Mock()
    mocker.patch(
        "openbb_terminal.stocks.prediction_techniques.pred_controller." + mocked_func,
        new=mock,
    )
