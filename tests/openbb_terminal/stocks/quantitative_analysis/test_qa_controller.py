...
def test_menu_without_queue_completion(mocker):
    ...
    mocker.patch(
        target="openbb_terminal.stocks.quantitative_analysis.qa_controller.session",
    )
    mocker.patch(
        target="openbb_terminal.stocks.quantitative_analysis.qa_controller.session.prompt",
        return_value="quit",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [],
            dict(
                data=QA_CONTROLLER.stock[QA_CONTROLLER.target],
                limit=1,
                sortby="",
                descend=True,
                export="csv",
...
def test_call_func(
    ...
    if mocked_func:
        mock = mocker.Mock()
        mocker.patch(
            "openbb_terminal.stocks.quantitative_analysis.qa_controller." + mocked_func,
            new=mock,
        )
