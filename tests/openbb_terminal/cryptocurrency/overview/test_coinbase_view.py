...
from openbb_terminal.cryptocurrency.due_diligence import coinbase_view as dd_coinbase_view
from openbb_terminal.cryptocurrency.overview import coinbase_view as ov_coinbase_view
from openbb_terminal.portfolio.brokers.coinbase import coinbase_view as bro_coinbase_view
from tests.helpers.helpers import check_print


@check_print(assert_in="Open")
def test_display_candles(mocker):
    mock_request = mocker.patch(
        "openbb_terminal.cryptocurrency.due_diligence.coinbase_model.make_coinbase_request"
    )
    mock_validity = mocker.patch(
        "openbb_terminal.cryptocurrency.due_diligence.coinbase_model.check_validity_of_product"
    )
...
def test_display_trades(mocker):
    mock_request = mocker.patch(
        "openbb_terminal.cryptocurrency.due_diligence.coinbase_model.make_coinbase_request"
    )
    mock_validity = mocker.patch(
...
def test_display_orders(mocker):
    mock_request = mocker.patch(
        "openbb_terminal.portfolio.brokers.coinbase.coinbase_model.make_coinbase_request"
    )
    mock_validity = mocker.patch(
        "openbb_terminal.portfolio.brokers.coinbase.coinbase_model._check_account_validity"
    )
    mock_validity.return_value = "ETH"
    mock_request.return_value = [
        {
...
def test_display_deposits(mocker):
    mock_request = mocker.patch(
        "openbb_terminal.portfolio.brokers.coinbase.coinbase_model.make_coinbase_request"
    )
    mock_validity = mocker.patch(
...
def test_display_history(mocker):
    mock_request = mocker.patch(
        "openbb_terminal.portfolio.brokers.coinbase.coinbase_model.make_coinbase_request"
    )
    mock_validity = mocker.patch(
...
@pytest.mark.vcr
def test_display_trading_pairs():
