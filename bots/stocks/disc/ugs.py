...
def ugs_command(num: int = 10):
    """Show most undervalued growth stock. [Yahoo Finance]"""

    # Debug user input
    if cfg.DEBUG:
        logger.debug("disc ugs %s", num)
    ...
    # Retrieve data
    df = yahoofinance_model.get_ugs().head(num)
    ...
    # Output data
    title = "Stocks: Most Undervalued Growth Stock [Yahoo Finance]"
    ...
    if dindex > 15:
        ...
        fig.update_traces(cells=(dict(align=["center", "left"])))
        imagefile = helpers.save_image("disc-ugs.png", fig)
