...
def mock_processor():
    ...
    with patch("openbb_cli.controllers.platform_controller_factory.ArgparseClassProcessor") as mock:
...
def factory(platform_router, mock_processor):
    ...
    return PlatformControllerFactory(platform_router=platform_router, reference={"test": "ref"})


def test_init(mock_processor):
    """Test the initialization of the PlatformControllerFactory."""
    factory = PlatformControllerFactory(platform_router=MagicMock(), reference={"test": "ref"})
...
def test_create_controller(factory):
    ...
    assert "test_router_settings" not in [cmd.replace("test_router_", "") for cmd in ControllerClass.CHOICES_COMMANDS]
