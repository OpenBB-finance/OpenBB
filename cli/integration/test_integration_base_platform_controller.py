...
def platform_controller():
    ...
    translators["test_command"]._parser = Mock(_actions=[Mock(dest="data", choices=[], type=str, nargs=None)])
    translators["test_command"].execute_func = Mock(return_value=Mock())
    translators["test_menu"]._parser = Mock(_actions=[Mock(dest="data", choices=[], type=str, nargs=None)])
    translators["test_menu"].execute_func = Mock(return_value=Mock())

    controller = PlatformController(name="test", parent_path=["platform"], translators=translators)
    return controller
...
def test_platform_controller_initialization(platform_controller):
    ...
    assert expected_path == platform_controller.PATH, "Controller path was not set correctly"
...
def test_command_generation(platform_controller):
    ...
    assert hasattr(platform_controller, command_method_name), "Command method was not created"


@patch("openbb_cli.controllers.base_platform_controller.PlatformController._link_obbject_to_data_processing_commands")
@patch("openbb_cli.controllers.base_platform_controller.PlatformController._generate_commands")
@patch("openbb_cli.controllers.base_platform_controller.PlatformController._generate_sub_controllers")
@pytest.mark.integration
def test_platform_controller_calls(mock_sub_controllers, mock_commands, mock_link_commands):
    ...
    _ = PlatformController(name="test", parent_path=["platform"], translators=translators)
