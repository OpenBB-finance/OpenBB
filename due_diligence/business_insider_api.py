...
def estimates(l_args, s_ticker):
    parser = argparse.ArgumentParser(prog='est',
                                     description="""Yearly estimates and quarter earnings/revenues [Source: Business
                                     Insider]""")
    ...
    try:
        ...
        for estimates_quarter_metric in text_soup_market_business_insider.findAll('td', {
            'class': 'table__td font-color-dim-gray'}):
        ...
        for idx, metric_value in enumerate(
                text_soup_market_business_insider.findAll('td', {'class': 'table__td text-right'})):
        ...
        df_quarter_earnings = pd.DataFrame.from_dict(d_metric_quarter_earnings, orient='index',
                                                     columns=l_estimates_quarter_header)

        # df_quarter_earnings.index.name = 'Earnings'
        df_quarter_revenues = pd.DataFrame.from_dict(d_metric_quarter_revenues, orient='index',
                                                     columns=l_estimates_quarter_header)
        ...
        print(
            text_soup_market_business_insider.find('div', {'class': "text_right instrument-description"}).text.strip())
        print("")
...
def insider_activity(l_args, df_stock, s_ticker, s_start, s_interval):
    ...
    try:
        ...
        for idx, insider_val in enumerate(
                text_soup_market_business_insider.findAll('td', {'class': "table__td text-center"})):
        ...
        df_insider['Trade'] = df_insider.apply(
            lambda row: (1, -1)[row.Type == 'Sell'] * float(row['Shares Traded'].replace(',', '')), axis=1)
        plt.xlim(df_insider.index[0], df_stock.index[-1])
        min_price, max_price = ax.get_ylim()

        price_range = max_price - min_price
        shares_range = df_insider[df_insider['Type'] == 'Buy'].groupby(by=['Date']).sum()['Trade'].max() - \
                       df_insider[df_insider['Type'] == 'Sell'].groupby(by=['Date']).sum()['Trade'].min()
        ...
        for ind in df_insider[df_insider['Type'] == 'Sell'].groupby(by=['Date']).sum().index:
            ...
            plt.vlines(x=ind_dt,
                       ymin=n_stock_price + n_proportion * float(
                               df_insider[df_insider['Type'] == 'Sell'].groupby(by=['Date']).sum()['Trade'][ind]),
                       ymax=n_stock_price,
                       colors='red', ls='-', lw=5)
        ...
        for ind in df_insider[df_insider['Type'] == 'Buy'].groupby(by=['Date']).sum().index:
            ...
            plt.vlines(x=ind_dt,
                       ymin=n_stock_price,
                       ymax=n_stock_price + n_proportion * float(
                               df_insider[df_insider['Type'] == 'Buy'].groupby(by=['Date']).sum()['Trade'][ind]),
